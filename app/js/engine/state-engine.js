...
var StateEngine = (function() {
    ...
    function getLibraryDay(nowMs) {
        var shifted = new Date((typeof nowMs === 'number' ? nowMs : Date.now()) + (3 * 60 * 60 * 1000));
        return shifted.getUTCFullYear() + '-' +
            String(shifted.getUTCMonth() + 1).padStart(2, '0') + '-' +
            String(shifted.getUTCDate()).padStart(2, '0');
    }

    function getLibraryMidnightDelay(nowMs) {
        var now = typeof nowMs === 'number' ? nowMs : Date.now();
        var shifted = now + (3 * 60 * 60 * 1000);
        var nextMidnight = (Math.floor(shifted / 86400000) + 1) * 86400000;
    ...
    function _getLibraryChapterConfig(chapter) {
        ...
        if (chapter === 'chapter4') {
            return { cost: libraryCfg.CHAPTER_FOUR_COST, memoPrefix: libraryCfg.CHAPTER_FOUR_MEMO_PREFIX };
        }
        if (chapter === 'chapter5') {
        ...
        if (chapter === 'chapter7') {
            return { cost: libraryCfg.CHAPTER_SEVEN_COST, memoPrefix: libraryCfg.CHAPTER_SEVEN_MEMO_PREFIX };
        }
        if (chapter === 'chapter8') {
            return { cost: libraryCfg.CHAPTER_EIGHT_COST, memoPrefix: libraryCfg.CHAPTER_EIGHT_MEMO_PREFIX };
        }
        return null;
    }
    ...
    function _collectLibraryUnlockPayments(awards) {
        var payments = {};
        var libraryCfg = cfg.LIBRARY || {};
        var chapters = ['chapter2', 'chapter3', 'chapter4', 'chapter5', 'chapter6', 'chapter7', 'chapter8'];
    ...
    function _handleLibraryUnlock(account, blockNum, chapter, day) {
        ...
        return {
            type: chapter === 'chapter2' ? 'library_chapter_two_unlocked' :
                ...
                chapter === 'chapter5' ? 'library_chapter_five_unlocked' :
                chapter === 'chapter6' ? 'library_chapter_six_unlocked' :
                chapter === 'chapter7' ? 'library_chapter_seven_unlocked' : 'library_chapter_eight_unlocked',
            account: account,
            chapter: chapter,
            day: day,
    ...
    function hasLibraryAccess(account, chapter, day) {
        var access = account && chapter && worldState.libraryAccess &&
