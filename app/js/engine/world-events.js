...
var WorldEvents = (function() {
    ...
    var SKY_TWISTS = [
        'Летний снегопад прикрыл город от жары, но ненадолго, до первого рассвета...',
    ...
    var FESTIVAL_TWISTS = [
        'у дверей оставляют тёплое слово, и дом отвечает скрипом половиц.',
        ...
        'завтра мир обещает новую странность и честно держит паузу.'
    ];


    var NATURE_PAGES = [
        { icon: '🪶', titleKey: 'home_magic_nature_title', text: 'Миграция безликих птиц приобрела угрожающие масштабы. Компас повернулся к ним спиной. Над территориями Запредельного Острова запрещена левитация...' },
        ...
        { icon: '🪨', titleKey: 'home_magic_nature_title', text: 'Камни у старой дороги меняют места, чтобы путники думали, будто дорога стала умнее. Карты мира делают вид, что знали это заранее.' },
        { icon: '🌾', titleKey: 'home_magic_nature_title', text: 'Колосья на полях слушают ветер и записывают прогноз прямо на собственных стеблях. Ветер унёс возражения в сторону арены.' },
        { icon: '🦌', titleKey: 'home_magic_nature_title', text: 'Олени тумана выходят только к тем, кто умеет не смотреть слишком настойчиво.' }
    ];

    var LEGEND_PAGES = [
        { icon: '🐉', titleKey: 'home_world_legends_title', text: 'В одно из пришествий Эфирного Дракона Цветомудрец улыбнулся и превратил его в радугу. С тех пор дракон боится радуги, а маски дракона примеряют даже те, кто уверяет, что совсем не боится.' },
        ...
        { icon: '🌅', titleKey: 'home_world_legends_title', text: 'Следующая проверка назначена на очень странный рассвет.' },
        { icon: '🐉', titleKey: 'home_world_legends_title', text: 'Дракон, вероятно, отрицает причастность.' },
        { icon: '🎒', titleKey: 'home_world_legends_title', text: 'Сумки игроков делают вид, что это их не касается.' }
    ];
    ...
    var SPELL_PAGES = [
        ...
        { icon: '🕯️', titleKey: 'home_magic_spells_title', text: 'Свеча не гори, но свет сохрани — тень обмани и дверь поверни; пророческая пыль легла на последнюю точку. Если текст кажется бредом, значит защита работает.' }
    ];
    ...
    function getCurrentSky(blockNum) {
        ...
        var twist = SKY_TWISTS[day % SKY_TWISTS.length];
        var out = {};
