...
test('magical weather is labelled and affects hunts', function () {
  ...
  assert.ok(/i18n\/ru.js\?v=20260731g/.test(indexHtml), 'Russian weather copy must be cache-busted');
  assert.ok(/i18n\/en.js\?v=20260731g/.test(indexHtml), 'English weather copy must be cache-busted');
  assert.ok(/home.js\?v=20260731g/.test(indexHtml), 'home forecast layout must be cache-busted');
  assert.ok(/js\/ui\/screens\/quests.js\?v=20260730c/.test(indexHtml), 'quest-limit UX must be cache-busted');
  assert.ok(/nav.js\?v=20260731g/.test(indexHtml), 'bottom tray nav must be cache-busted');
  assert.ok(/leaderboard.js\?v=20260731g/.test(indexHtml), 'leaderboard icon motion must be cache-busted');
  assert.ok(/world-events.js\?v=20260731g/.test(indexHtml), 'world events news and festival copy must be cache-busted');
  assert.ok(/main.css\?v=20260731g/.test(indexHtml), 'forecast grid CSS must be cache-busted');
...
test('character screen uses current home-scale vitals and growth explainers', function () {
  assert.ok(/character.js\?v=20260731g/.test(indexHtml), 'character screen should be cache-busted');
...
test('hunt screen exposes explicit camp rest promised by Help', function () {
  assert.ok(/hunt.js\?v=20260731g/.test(indexHtml), 'hunt screen should be cache-busted');
  assert.ok(/broadcast.js\?v=20260731c/.test(indexHtml), 'broadcast helper should be cache-busted for restAction');
  assert.ok(/state-engine.js\?v=20260731c/.test(indexHtml), 'state-engine should be cache-busted for processRestResult');
...
test('narrator voice preferences support gender and timbre', function () {
  ...
  assert.ok(/settings.js\?v=20260731g/.test(indexHtml), 'settings should be cache-busted');
...
test('home action tiles reflect Denis priority order', function () {
  assert.ok(/home.js\?v=20260731g/.test(indexHtml), 'home screen should be cache-busted for action order');
...
test('character vital explainers are placed immediately after their bars', function () {
  assert.ok(/character.js\?v=20260731g/.test(indexHtml), 'character screen should be cache-busted for vital layout');
...
test('minor rift banner explains itself and is actionable', function () {
  assert.ok(/home.js\?v=20260731g/.test(indexHtml), 'home screen should be cache-busted for rift explanation');
...
test('hunt rest uses home-scale HP values', function () {
  assert.ok(/hunt.js\?v=20260731g/.test(indexHtml), 'hunt screen should be cache-busted for HP display scale');
...
test('marketplace groups identical sellable items and supports quantity listing', function () {
  assert.ok(/marketplace.js\?v=20260731g/.test(indexHtml), 'marketplace screen should be cache-busted');
...
test('hunt and arena icons are distinct', function () {
  assert.ok(/nav.js\?v=20260731g/.test(indexHtml), 'nav should be cache-busted for hunt icon');
...
test('hunt headings and help use updated thematic icons', function () {
  assert.ok(/hunt.js\?v=20260731g/.test(indexHtml), 'hunt screen should be cache-busted for heading icons');
  ...
  assert.ok(/help.js\?v=20260731a/.test(indexHtml), 'help screen should be cache-busted for hunt icon');
...
test('marketplace sell items have semantic item icons', function () {
  assert.ok(/marketplace.js\?v=20260731g/.test(indexHtml), 'marketplace screen should be cache-busted for sell icons');
...
test('world boss UI can enter active window from schedule even without spawn checkpoint', function () {
  ...
  assert.ok(/state-engine.js\?v=20260731c/.test(indexHtml), 'state engine should be cache-busted for boss attack spawn-block parity');
...
test('chronicle draft survives rerenders while feed loads', function () {
  assert.ok(/chronicle.js\?v=20260724b/.test(indexHtml), 'chronicle screen should be cache-busted for draft preservation');
...
test('v82 Denis feedback polish is explicit and cache-busted', function () {
  ...
  assert.ok(/home\.js\?v=20260731g/.test(indexHtml), 'Home should be cache-busted for v82');
  assert.ok(/world-events\.js\?v=20260731g/.test(indexHtml), 'world events should be cache-busted for v82');
  assert.ok(/hunt\.js\?v=20260731g/.test(indexHtml), 'Hunt should be cache-busted for Armageddon lock feedback');
  assert.ok(/settings\.js\?v=20260731g/.test(indexHtml), 'Settings should be cache-busted for sound icons');
  ...
  assert.ok(/member-icon vmagic-breathe/.test(guildJs) && /guild-my-rank[\s\S]*vmagic-breathe/.test(guildJs), 'guild rank icons including novice should breathe');
...
test('magical guide replaces extra magical pages tab without shuffling practical help', function () {
  ...
  assert.ok(/help\.js\?v=20260731a/.test(indexHtml), 'Help should be cache-busted for guide redesign');
  assert.ok(/main\.css\?v=20260731g/.test(indexHtml), 'main CSS should be cache-busted for guide redesign');
  assert.ok(/viz-magic-v111/.test(swJsV83), 'service worker should use the current v91 cache');
...
test('Denis v91 polish batch keeps quests fair and icons lively', () => {
  ...
  assert.ok(/main\.css\?v=20260731g/.test(indexHtml), 'main CSS should be cache-busted for v91 polish');
  assert.ok(/world-events\.js\?v=20260731g/.test(indexHtml), 'world-events should be cache-busted for v91 polish');
  ...
  assert.ok(/inventory\.js\?v=20260731g/.test(indexHtml) && /marketplace\.js\?v=20260731g/.test(indexHtml), 'item icon screens should be cache-busted');
  assert.ok(/guild\.js\?v=20260731g/.test(indexHtml) && /settings\.js\?v=20260731g/.test(indexHtml), 'guild/settings screens should be cache-busted');
  assert.ok(/viz-magic-v111/.test(swJs), 'service worker should use v91 cache');
});
...
test('profile avatars from VIZ json_metadata are bounded and optional', function () {
  ...
  assert.ok(/leaderboard\.js\?v=20260731g/.test(indexHtml), 'leaderboard UI should be cache-busted');
  assert.ok(/arena\.js\?v=20260731g/.test(indexHtml), 'arena UI should be cache-busted');
  ...
  assert.ok(/character\.js\?v=20260731g/.test(indexHtml), 'character UI should be cache-busted');
  assert.ok(/main\.css\?v=20260731g/.test(indexHtml), 'avatar CSS should be cache-busted');
  assert.ok(/viz-magic-v111/.test(swJs), 'service worker should use v92 cache');
});
...
test('safe avatar upload UI re-encodes before JSON_METADATA writes', function () {
  ...
  assert.ok(/settings\.js\?v=20260731g/.test(indexHtml), 'settings screen should be cache-busted for avatar upload');
  assert.ok(/js\/i18n\/ru\.js\?v=20260731g/.test(indexHtml) && /js\/i18n\/en\.js\?v=20260731g/.test(indexHtml), 'i18n should be cache-busted for avatar strings');
});
...
test('character profile refreshes avatar from VIZ JSON_METADATA on render', function () {
  ...
  assert.ok(/character\.js\?v=20260731g/.test(indexHtmlAvatarProfile), 'character screen should be cache-busted for live avatar refresh');
  assert.ok(/js\/i18n\/ru\.js\?v=20260731g/.test(indexHtmlAvatarProfile) && /js\/i18n\/en\.js\?v=20260731g/.test(indexHtmlAvatarProfile), 'i18n should be cache-busted for updated avatar copy');
});
...
test('Denis v97 world, inventory, nav, arena and guide polish is explicit', function () {
  ...
  assert.ok(/return \[\s*_dailyFromPool\(NATURE_PAGES/.test(worldEventsV97) && /_dailyFromPool\(LEGEND_PAGES/.test(worldEventsV97) && /_dailyFromPool\(SPELL_PAGES/.test(worldEventsV97), 'home should render nature, legend and spell living pages');
  ...
  assert.ok(/world-events\.js\?v=20260731g/.test(indexV97) && /home\.js\?v=20260731g/.test(indexV97) && /main\.css\?v=20260731g/.test(indexV97), 'v97 files should be cache-busted');
});
...
test('living pages use an existing daily page helper', function () {
  ...
  assert.ok(/world-events\.js\?v=20260731g/.test(indexV99), 'world-events should be cache-busted for living-page crash fix');
});
...
test('home startup does not fetch blockchain account before VIZ transport is ready', function () {
  ...
  assert.ok(/home\.js\?v=20260731g/.test(indexV101), 'Home screen should be cache-busted for safe startup mana refresh');
});
...
test('hunt habitat hides stale prey and labels dangerous areas instead of pretending mana is win chance', function () {
  ...
  assert.ok(/Сильные магические сущности\. Опасно!/.test(ruJs), 'Russian danger warning should match player request');
...
test('bottom navigation keeps icons above single-line labels and cache-busts changed assets', function () {
  ...
  assert.ok(/main\.css\?v=20260731g/.test(indexHtml), 'main.css cache bust missing');
  assert.ok(/hunt\.js\?v=20260731g/.test(indexHtml), 'hunt cache bust missing');
  assert.ok(/inventory\.js\?v=20260731g/.test(indexHtml), 'inventory cache bust missing');
  assert.ok(/marketplace\.js\?v=20260731g/.test(indexHtml), 'marketplace cache bust missing');
  assert.ok(/nav\.js\?v=20260731g/.test(indexHtml), 'nav cache bust missing');
  assert.ok(/viz-magic-v111/.test(read('app/sw.js')), 'service worker cache should be v103');
});
...
test('v103 weave surge, default avatars, and guide copy polish are explicit', function () {
  ...
  assert.ok(/main\.css\?v=20260731g/.test(indexHtml), 'v103 CSS should be cache-busted');
  assert.ok(/home\.js\?v=20260731g/.test(indexHtml) && /character\.js\?v=20260731g/.test(indexHtml), 'v103 Home and Character should be cache-busted');
  assert.ok(/settings\.js\?v=20260731g/.test(indexHtml) && /help\.js\?v=20260731a/.test(indexHtml), 'v103 Settings and Help should be cache-busted');
  assert.ok(/viz-magic-v111/.test(swJs), 'service worker should use v103 cache');
});
...
test('quest abandon charges only unstarted quests and warns before forfeit', function () {
  ...
  assert.ok(/js\/i18n\/ru\.js\?v=20260731g/.test(indexPenalty) && /js\/i18n\/en\.js\?v=20260731g/.test(indexPenalty), 'i18n cache bust missing for abandon penalty');
  assert.ok(/viz-magic-v111/.test(swPenalty), 'service worker should use v104 cache');
});
...
test('v110 player feedback keeps requested icons, copy, vital explainers, and event text placement explicit', function () {
  ...
  assert.ok(/creature-warning-text/.test(hunt), 'Rift Colossus should keep textual danger warning without triangle');
  ...
  assert.ok(/event-edit-copy/.test(home), 'Weave Surge and Minor Rift copy should be exposed for temporary text editing');
  ...
  ['main.css','ru.js','en.js','world-events.js','progress-bar.js','nav.js','home.js','character.js','hunt.js','inventory.js','arena.js','settings.js','leaderboard.js'].forEach(function(asset) {
    assert.ok(new RegExp(asset.replace('.', '\\.') + '\\?v=20260731g').test(index), asset + ' should be cache-busted for v110');
  });
  assert.ok(/viz-magic-v111/.test(sw), 'service worker should use v110 cache');
});
...
test('v109 player polish batch removes stale text, fixes motion/copy/icons, and fills hunt tier gap', function () {
  ...
  assert.ok(/creature-name[\s\S]*creature-level[\s\S]*creature-warning-text/.test(huntV109), 'hunt card should keep level on the first row before the textual danger warning');
  assert.ok(/hunt_danger_warning: 'Сильные магические сущности\. Опасно!'/.test(ruV109), 'RU danger warning should start with capital Сильные and omit Здесь очень');
  ...
  assert.ok(/stonewarden: 'Каменный страж'/.test(characterV109), 'Character title should keep readable RU class name');
  ...
  ['main.css','ru.js','en.js','world-events.js','creatures.js','character.js','hunt.js','inventory.js','arena.js','guild.js','marketplace.js','leaderboard.js','settings.js'].forEach(function(asset) {
    assert.ok(new RegExp(asset.replace('.', '\\.') + '\\?v=20260731g').test(indexV109), asset + ' should be cache-busted for v109');
  });
  assert.ok(/viz-magic-v111/.test(swV109), 'service worker should use v109 cache');
});
...
test('v108 inventory, bottom nav, hunt danger marker, and arena motion polish are explicit', function () {
  ...
  assert.ok(/creature-warning-text/.test(huntV108), 'Hunt should keep textual danger warning without a yellow oval or triangle');
  ...
  assert.ok(/main\.css\?v=20260731g/.test(indexV108), 'main CSS cache bust missing for v108');
  assert.ok(/inventory\.js\?v=20260731g/.test(indexV108), 'inventory cache bust missing for v108');
  assert.ok(/hunt\.js\?v=20260731g/.test(indexV108), 'hunt cache bust missing for v108');
  assert.ok(/arena\.js\?v=20260731g/.test(indexV108), 'arena cache bust missing for v108');
  assert.ok(/nav\.js\?v=20260731g/.test(indexV108), 'nav cache bust missing for v108');
  assert.ok(/marketplace\.js\?v=20260731g/.test(indexV108), 'marketplace cache bust missing for v108 icon parity');
  assert.ok(/js\/i18n\/ru\.js\?v=20260731g/.test(indexV108) && /js\/i18n\/en\.js\?v=20260731g/.test(indexV108), 'i18n cache bust missing for v108 item labels');
  assert.ok(/viz-magic-v111/.test(swV108), 'service worker should use v108 cache');
});
...
test('v107 hunt combat uses spell mana cost, not full account energy', function () {
  ...
  assert.ok(/combat\.js\?v=20260731c/.test(indexV107), 'combat cache bust missing for v107 hunt energy fix');
  assert.ok(/state-engine\.js\?v=20260731c/.test(indexV107), 'state engine cache bust missing for v107 hunt energy fix');
  assert.ok(/hunt\.js\?v=20260731g/.test(indexV107), 'hunt screen cache bust missing for v107 hunt energy fix');
  assert.ok(/viz-magic-v111/.test(swV107), 'service worker should use v107 cache');
});
...
test('v106 hunt danger copy is truthful and Weave title is single-line', function () {
  ...
  assert.ok(/return creature\.deadly === true/.test(huntV106), 'danger label should only appear on explicit deadly targets');
  assert.ok(/main\.css\?v=20260731g/.test(indexV106), 'v106 CSS cache bust missing');
  assert.ok(/creatures\.js\?v=20260731g/.test(indexV106), 'v106 creatures cache bust missing');
  assert.ok(/hunt\.js\?v=20260731g/.test(indexV106), 'v106 hunt cache bust missing');
  assert.ok(/viz-magic-v111/.test(swV106), 'service worker should use v106 cache');
});
...
test('v105 text quality, season colors, avatar title, and spell modal polish are explicit', function () {
  ...
  assert.ok(/main\.css\?v=20260731g/.test(indexV105), 'main CSS cache bust missing for v105');
  assert.ok(/home\.js\?v=20260731g/.test(indexV105) && /character\.js\?v=20260731g/.test(indexV105), 'Home/Character cache bust missing for v105');
  assert.ok(/settings\.js\?v=20260731g/.test(indexV105) && /leaderboard\.js\?v=20260731g/.test(indexV105), 'Settings/Leaderboard cache bust missing for v105');
  assert.ok(/world-events\.js\?v=20260731g/.test(indexV105), 'world events cache bust missing for v105');
  assert.ok(/js\/i18n\/ru\.js\?v=20260731g/.test(indexV105) && /js\/i18n\/en\.js\?v=20260731g/.test(indexV105), 'i18n cache bust missing for v105');
  assert.ok(/viz-magic-v111/.test(swV105), 'service worker should use v105 cache');
});


test('v111 player feedback removes repeated tails and stabilizes icons and character math copy', function () {
  const index = read('app/index.html');
  const sw = read('app/sw.js');
  const css = read('app/css/main.css');
  const worldEvents = read('app/js/engine/world-events.js');
  ...
  assert.strictEqual((worldEvents.match(/Ветер несёт запах костра/g) || []).length, 1, 'campfire-smell phrase should exist once across queued text blocks');
  ...
  assert.ok(/item\.type === 'flame_votive_mark' \|\| item\.type === 'altar_spark'/.test(inventory), 'both Altar Spark item types should suppress rarity diamond before name');
  ...
  assert.ok(/body\[data-icon-motion="sparkle"\] \.item-card \.item-icon[\s\S]*animation-duration: 5\.0s/.test(css), 'bag icons should use final slow soft-breathe override');
  assert.ok(/body\[data-icon-motion="sparkle"\] \.arena-screen \.arena-class-icon[\s\S]*animation-duration: 5\.0s/.test(css), 'arena icons should use final slow soft-breathe override');
  assert.ok(/body\[data-icon-motion="sparkle"\] \.leaderboard-title-icon[\s\S]*animation-duration: 5\.0s/.test(css), 'leaderboard icon should use final slow soft-breathe override');
  assert.ok(/#bottom-nav \.nav-icon \{ min-height: 1\.9rem/.test(css), 'bottom nav labels should align despite larger Character avatar');
  ...
  assert.ok(/function _renderPlayerIdentity/.test(leaderboard) && /name && name !== account/.test(leaderboard), 'leaderboard should avoid naked duplicate account names');

  assert.ok(/char_hp_explainer/.test(character) && /char_xp_explainer/.test(character), 'HP and XP descriptions should be visible again');
  ...
  assert.ok(/Каменный Страж ×1\.30, Огнеплёт ×0\.90, Лунный Странник ×1\.00, Цветомудрец ×1\.10/.test(ru), 'HP explanation should use numeric class multipliers in RU');
  ...
  assert.ok(/_statRow\(t\('char_potency'\), ch\.pot \|\| 0, corePerStat, 0, totalPot\)/.test(character) && /stat-formula/.test(character), 'character stats should show base + core + equipment formula before total');
  assert.ok(!/t\('class_' \+ ch\.className\) \+ ' \\u2022 '/.test(character), 'class subtitle under the shield should be removed');
  assert.ok(/event-edit-copy/.test(css) && /border: 1px dashed/.test(css), 'Weave Surge and Minor Rift edit preview should be visually opened');
  ...
  ['main.css','ru.js','en.js','world-events.js','progress-bar.js','nav.js','home.js','character.js','hunt.js','inventory.js','arena.js','settings.js','leaderboard.js','marketplace.js','guild.js','creatures.js'].forEach(function(asset) {
    assert.ok(new RegExp(asset.replace('.', '\\.') + '\\?v=20260731g').test(index), asset + ' should be cache-busted for v111');
  });
  assert.ok(/viz-magic-v111/.test(sw), 'service worker should use v111 cache');
});
