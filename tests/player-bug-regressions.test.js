...
test('hunt headings and help use updated thematic icons', function () {
  ...
  assert.ok(/help.js\?v=20260826x/.test(indexHtml), 'help screen should be cache-busted for hunt icon');
...
test('magical library preserves legacy lore links while artwork is rebuilt', function () {
  const helpJs = read('app/js/ui/screens/help.js');
  ...
  assert.ok(!/help-library-map-image|help-library-zoom-toggle/.test(helpJs), 'temporary library mode should not render artwork or zoom controls');
  assert.ok(/help\.js\?v=20260826x/.test(index), 'help screen should be cache-busted for text-only Magical Library');
  ...
  assert.ok(/viz-magic-v162/.test(sw), 'service worker should publish the text-only library cache bump');
});
...
test('magical guide replaces extra magical pages tab without shuffling practical help', function () {
  ...
  assert.ok(/help\.js\?v=20260826x/.test(indexHtml), 'Help should be cache-busted for guide redesign');
  assert.ok(/main\.css\?v=20260826u/.test(indexHtml), 'main CSS should be cache-busted for guide redesign');
  assert.ok(/viz-magic-v162/.test(swJsV83), 'service worker should use the current v91 cache');
...
test('Denis v91 polish batch keeps quests fair and icons lively', () => {
  ...
  assert.ok(/viz-magic-v162/.test(swJs), 'service worker should use v91 cache');
});
...
test('profile avatars from VIZ json_metadata are bounded and optional', function () {
  ...
  assert.ok(/viz-magic-v162/.test(swJs), 'service worker should use v92 cache');
});
...
test('bottom navigation keeps icons above single-line labels and cache-busts changed assets', function () {
  ...
  assert.ok(/viz-magic-v162/.test(read('app/sw.js')), 'service worker cache should be v103');
});
...
test('v103 weave surge, default avatars, and guide copy polish are explicit', function () {
  ...
  assert.ok(/settings\.js\?v=20260826k/.test(indexHtml) && /help\.js\?v=20260826x/.test(indexHtml), 'v103 Settings and Help should be cache-busted');
  assert.ok(/viz-magic-v162/.test(swJs), 'service worker should use v103 cache');
});
...
test('quest abandon charges only unstarted quests and warns before forfeit', function () {
  ...
  assert.ok(/viz-magic-v162/.test(swPenalty), 'service worker should use v104 cache');
});
...
test('v110 player feedback keeps requested icons, copy, vital explainers, and event text placement explicit', function () {
  ...
  assert.ok(/viz-magic-v162/.test(sw), 'service worker should use v110 cache');
});
...
test('v109 player polish batch removes stale text, fixes motion/copy/icons, and fills hunt tier gap', function () {
  ...
  assert.ok(/viz-magic-v162/.test(swV109), 'service worker should use v109 cache');
});
...
test('v108 inventory, bottom nav, hunt danger marker, and arena motion polish are explicit', function () {
  ...
  assert.ok(/viz-magic-v162/.test(swV108), 'service worker should use v108 cache');
});
...
test('v107 hunt combat uses spell mana cost, not full account energy', function () {
  ...
  assert.ok(/viz-magic-v162/.test(swV107), 'service worker should use v107 cache');
});
...
test('v106 hunt danger copy is truthful and Weave title is single-line', function () {
  ...
  assert.ok(/viz-magic-v162/.test(swV106), 'service worker should use v106 cache');
});
...
test('v105 text quality, season colors, avatar title, and spell modal polish are explicit', function () {
  ...
  assert.ok(/viz-magic-v162/.test(swV105), 'service worker should use v105 cache');
});
...
test('v112 player feedback fixes event surface weather inventory and ranking polish', function () {
  ...
  assert.ok(/viz-magic-v162/.test(sw), 'service worker should use v112 cache');
});
...
test('v114 loading state does not invent an embercaster level-one character', function () {
  ...
  assert.ok(/viz-magic-v162/.test(swJs), 'service worker should publish v114');
});
...
test('v115 service worker forces stale PWA windows onto the fresh cache-busted app shell', function () {
  ...
  assert.ok(/viz-magic-v162/.test(swJs), 'service worker should publish v115');
});
...
test('production PWA install is available before login and landing icons do not depend on emoji fonts', function () {
  ...
  assert.ok(/viz-magic-v162/.test(swJs), 'service-worker cache should advance for fresh runtime assets');
});
...
test('installed app cold-start has visible fallback and service-worker network timeouts', function () {
  ...
  assert.ok(/viz-magic-v162/.test(swJs), 'service worker cache should advance for cold-start black-screen fix');
...
test('verified world maps and font-independent SVG icons remain intact', function () {
  ...
  assert.ok(/help\.js\?v=20260826x/.test(indexHtml), 'Help bundle should be cache-busted for text-only library mode');
  assert.ok(/viz-magic-v162/.test(swJs), 'service worker cache should advance for text-only library mode');
  assert.ok(/main\.css\?v=20260826u/.test(indexHtml), 'SVG icon styles should stay cache-busted');
});

test('Magical Library temporarily renders lore only while clean maps are rebuilt', function () {
  const helpJs = read('app/js/ui/screens/help.js');
  const swJs = read('app/sw.js');
  ...
  assert.ok(!/library-maps|help-library-map-image|help-library-map-viewport|help-library-zoom-toggle/.test(helpJs), 'temporary library dialog must not render or request map images');
  assert.ok(/help\.js\?v=20260826x/.test(indexHtml), 'text-only Help bundle should be cache-busted');
  assert.ok(/viz-magic-v162/.test(swJs), 'service worker cache should advance for text-only library mode');
});
