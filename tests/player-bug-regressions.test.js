...
test('hunt headings and help use updated thematic icons', function () {
  ...
  assert.ok(/help.js\?v=20260826y/.test(indexHtml), 'help screen should be cache-busted for hunt icon');
...
test('magical library renders the verified clean artwork set with full lore', function () {
  const helpJs = read('app/js/ui/screens/help.js');
  ...
  assert.ok(/assets\/library-maps-v3\/map-' \+ entry\.id \+ '\.jpg\?v=' \+ HELP_LIBRARY_ASSET_VERSION/.test(helpJs), 'library modal should request only the new v3 artwork path');
  assert.ok(/help-library-map-image/.test(helpJs) && /help-library-zoom-toggle/.test(helpJs), 'verified library artwork should restore image and zoom controls');
  assert.ok(!/library-maps-v2/.test(helpJs), 'library runtime must not reference the rejected v2 artwork');
  assert.strictEqual(fs.readdirSync(path.join(root, 'app/assets/library-maps-v3')).filter(name => /^map-[a-z_]+\.jpg$/.test(name)).length, 15, 'clean library set should contain exactly 15 JPEG files');
  assert.ok(/help\.js\?v=20260826y/.test(index), 'help screen should be cache-busted for clean Magical Library artwork');
  ...
  assert.ok(/viz-magic-v163/.test(sw), 'service worker should publish the clean library cache bump');
});
...
test('magical guide replaces extra magical pages tab without shuffling practical help', function () {
  ...
  assert.ok(/help\.js\?v=20260826y/.test(indexHtml), 'Help should be cache-busted for guide redesign');
  assert.ok(/main\.css\?v=20260826u/.test(indexHtml), 'main CSS should be cache-busted for guide redesign');
  assert.ok(/viz-magic-v163/.test(swJsV83), 'service worker should use the current v91 cache');
...
test('Denis v91 polish batch keeps quests fair and icons lively', () => {
  ...
  assert.ok(/viz-magic-v163/.test(swJs), 'service worker should use v91 cache');
});
...
test('profile avatars from VIZ json_metadata are bounded and optional', function () {
  ...
  assert.ok(/viz-magic-v163/.test(swJs), 'service worker should use v92 cache');
});
...
test('bottom navigation keeps icons above single-line labels and cache-busts changed assets', function () {
  ...
  assert.ok(/viz-magic-v163/.test(read('app/sw.js')), 'service worker cache should be v103');
});
...
test('v103 weave surge, default avatars, and guide copy polish are explicit', function () {
  ...
  assert.ok(/settings\.js\?v=20260826k/.test(indexHtml) && /help\.js\?v=20260826y/.test(indexHtml), 'v103 Settings and Help should be cache-busted');
  assert.ok(/viz-magic-v163/.test(swJs), 'service worker should use v103 cache');
});
...
test('quest abandon charges only unstarted quests and warns before forfeit', function () {
  ...
  assert.ok(/viz-magic-v163/.test(swPenalty), 'service worker should use v104 cache');
});
...
test('v110 player feedback keeps requested icons, copy, vital explainers, and event text placement explicit', function () {
  ...
  assert.ok(/viz-magic-v163/.test(sw), 'service worker should use v110 cache');
});
...
test('v109 player polish batch removes stale text, fixes motion/copy/icons, and fills hunt tier gap', function () {
  ...
  assert.ok(/viz-magic-v163/.test(swV109), 'service worker should use v109 cache');
});
...
test('v108 inventory, bottom nav, hunt danger marker, and arena motion polish are explicit', function () {
  ...
  assert.ok(/viz-magic-v163/.test(swV108), 'service worker should use v108 cache');
});
...
test('v107 hunt combat uses spell mana cost, not full account energy', function () {
  ...
  assert.ok(/viz-magic-v163/.test(swV107), 'service worker should use v107 cache');
});
...
test('v106 hunt danger copy is truthful and Weave title is single-line', function () {
  ...
  assert.ok(/viz-magic-v163/.test(swV106), 'service worker should use v106 cache');
});
...
test('v105 text quality, season colors, avatar title, and spell modal polish are explicit', function () {
  ...
  assert.ok(/viz-magic-v163/.test(swV105), 'service worker should use v105 cache');
});
...
test('v112 player feedback fixes event surface weather inventory and ranking polish', function () {
  ...
  assert.ok(/viz-magic-v163/.test(sw), 'service worker should use v112 cache');
});
...
test('v114 loading state does not invent an embercaster level-one character', function () {
  ...
  assert.ok(/viz-magic-v163/.test(swJs), 'service worker should publish v114');
});
...
test('v115 service worker forces stale PWA windows onto the fresh cache-busted app shell', function () {
  ...
  assert.ok(/viz-magic-v163/.test(swJs), 'service worker should publish v115');
});
...
test('production PWA install is available before login and landing icons do not depend on emoji fonts', function () {
  ...
  assert.ok(/viz-magic-v163/.test(swJs), 'service-worker cache should advance for fresh runtime assets');
});
...
test('installed app cold-start has visible fallback and service-worker network timeouts', function () {
  ...
  assert.ok(/viz-magic-v163/.test(swJs), 'service worker cache should advance for cold-start black-screen fix');
...
test('verified world maps and font-independent SVG icons remain intact', function () {
  ...
  assert.ok(/help\.js\?v=20260826y/.test(indexHtml), 'Help bundle should be cache-busted for text-only library mode');
  assert.ok(/viz-magic-v163/.test(swJs), 'service worker cache should advance for text-only library mode');
  assert.ok(/main\.css\?v=20260826u/.test(indexHtml), 'SVG icon styles should stay cache-busted');
});

test('Magical Library uses only the verified v3 artwork with lore and Close', function () {
  const helpJs = read('app/js/ui/screens/help.js');
  const swJs = read('app/sw.js');
  ...
  assert.ok(/library-maps-v3/.test(helpJs) && /help-library-map-image|help-library-map-viewport|help-library-zoom-toggle/.test(helpJs), 'verified library dialog should render v3 artwork and zoom controls');
  assert.ok(!/library-maps-v2/.test(helpJs), 'rejected v2 artwork must stay unreachable from runtime');
  assert.ok(/help\.js\?v=20260826y/.test(indexHtml), 'clean-art Help bundle should be cache-busted');
  assert.ok(/viz-magic-v163/.test(swJs), 'service worker cache should advance for clean library artwork');
});
