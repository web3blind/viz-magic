...
test('service worker updates quickly and keeps navigations network-first', function () {
  ...
  assert.ok(/viz-magic-v72/.test(swJs), 'service worker cache version should be bumped');
...
test('mobile entry helpers cover keyboard paste, home-screen shortcut, nav parity, non-intrusive toasts, and chronicle post dedupe', function () {
  ...
  assert.ok(/viz-magic-v72/.test(read('app/sw.js')), 'service worker cache should be bumped for UI changes');
});
...
test('music volume, narrator speech, and PWA icons are durable', function () {
  ...
  assert.ok(/manifest\.json\?v=20260718b/.test(indexHtml), 'manifest should be cache-busted for updated icon');
  assert.ok(/favicon\.ico\?v=20260712c/.test(indexHtml), 'favicon should be explicit for browser shortcut fallback');
  assert.ok(/viz-magic-v72-192\.png\?v=20260718b/.test(indexHtml), 'launcher icon link should be cache-busted');
  assert.ok(/assets\/icons\/viz-magic-v72-512\.png/.test(read('app/sw.js')), 'service worker should cache PWA launcher icons');
  assert.ok(/viz-magic-v72-512\.png/.test(read('app/manifest.json')), 'manifest should reference new icon URLs to bypass OS icon cache');
});
...
test('PWA icon and HP heart use expressive color accents', function () {
  assert.ok(/viz-magic-v72-192\.png\?v=20260718b/.test(indexHtml), 'PWA icon link should be cache-busted after expressive icon update');
  assert.ok(/viz-magic-v72/.test(read('app/manifest.json')), 'manifest start URL should change so launchers can refresh icons');
...
test('v72 PWA icon restores the previous launcher visual', function () {
  assert.ok(fs.existsSync(path.join(root, 'app/assets/icons/viz-magic-v72-192.png')), 'v72 192px restored icon should exist');
  assert.ok(fs.existsSync(path.join(root, 'app/assets/icons/viz-magic-v72-512.png')), 'v72 512px restored icon should exist');
  assert.ok(/viz-magic-v72-192\.png\?v=20260718b/.test(indexHtml), 'index should point at the v71 launcher icon');
  assert.ok(/"start_url":\s*"\/\?pwa=viz-magic-v72"/.test(read('app/manifest.json')), 'manifest start_url should force OS launcher refresh');
  assert.ok(/APP_SHELL_ASSETS[\s\S]*viz-magic-v72-512\.png/.test(read('app/sw.js')), 'fast install shell should still include restored launcher icons');
});

test('Denis feedback item and motion icons are semantic', function () {
  assert.ok(/viz-magic-v72-192\.png\?v=20260718b/.test(indexHtml), 'launcher icon should use the restored previous v72 launcher icon');
  assert.ok(/assets\/icons\/viz-magic-v72-512\.png/.test(read('app/sw.js')), 'service worker should cache v71 icon');
