...
test('map regions separate safe travel from gated current-region exploration', function () {
  ...
  assert.ok(/viz-magic-v189/.test(swJs), 'service worker cache should advance for the map action layout');
});
...
test('magical guide explains versioned XP thresholds without claiming a 100-percent mana level-up', function () {
  ...
  assert.ok(/С блока 83 000 000[\s\S]*1000 XP[\s\S]*1250 XP[\s\S]*1750 XP[\s\S]*2500 XP[\s\S]*3500 XP[\s\S]*4750 XP[\s\S]*6250 XP[\s\S]*8000 XP/.test(ruJs), 'the guide should publish the v2 activation boundary and approved thresholds');
  assert.ok(/Достигнутый уровень и уже заработанная часть следующего уровня сохраняются/.test(ruJs), 'the guide should explain the no-downgrade migration');
  assert.ok(/Старые действия до блока активации воспроизводятся по прежней шкале/.test(ruJs), 'the guide should explain deterministic historical replay');
  assert.ok(/Суммарно потраченные 100% Маны сами по себе не повышают уровень/.test(ruJs), 'the guide should explicitly reject the unapproved 100-percent-mana rule');
...
test('magical library restores the original board-game artwork with full lore', function () {
  ...
  assert.ok(/viz-magic-v189/.test(sw), 'service worker should publish the restored library cache bump');
});
...
test('magical guide replaces extra magical pages tab without shuffling practical help', function () {
  ...
  assert.ok(/viz-magic-v189/.test(swJsV83), 'service worker should use the current v91 cache');
...
test('Denis v91 polish batch keeps quests fair and icons lively', () => {
  ...
  assert.ok(/viz-magic-v189/.test(swJs), 'service worker should use v91 cache');
});
...
test('profile avatars from VIZ json_metadata are bounded and optional', function () {
  ...
  assert.ok(/viz-magic-v189/.test(swJs), 'service worker should use v92 cache');
});
...
test('bottom navigation keeps icons above single-line labels and cache-busts changed assets', function () {
  ...
  assert.ok(/viz-magic-v189/.test(read('app/sw.js')), 'service worker cache should be v103');
});
...
test('v103 weave surge, default avatars, and guide copy polish are explicit', function () {
  ...
  assert.ok(/viz-magic-v189/.test(swJs), 'service worker should use v103 cache');
});
...
test('quest abandon charges only unstarted quests and warns before forfeit', function () {
  ...
  assert.ok(/viz-magic-v189/.test(swPenalty), 'service worker should use v104 cache');
});
...
test('v110 player feedback keeps requested icons, copy, vital explainers, and event text placement explicit', function () {
  ...
  assert.ok(/viz-magic-v189/.test(sw), 'service worker should use v110 cache');
});
...
test('v109 player polish batch removes stale text, fixes motion/copy/icons, and fills hunt tier gap', function () {
  ...
  assert.ok(/viz-magic-v189/.test(swV109), 'service worker should use v109 cache');
});
...
test('v108 inventory, bottom nav, hunt danger marker, and arena motion polish are explicit', function () {
  ...
  assert.ok(/viz-magic-v189/.test(swV108), 'service worker should use v108 cache');
});
...
test('v107 hunt combat uses spell mana cost, not full account energy', function () {
  ...
  assert.ok(/viz-magic-v189/.test(swV107), 'service worker should use v107 cache');
});
...
test('v106 hunt danger copy is truthful and Weave title is single-line', function () {
  ...
  assert.ok(/viz-magic-v189/.test(swV106), 'service worker should use v106 cache');
});
...
test('v105 text quality, season colors, avatar title, and spell modal polish are explicit', function () {
  ...
  assert.ok(/viz-magic-v189/.test(swV105), 'service worker should use v105 cache');
});
...
test('v112 player feedback fixes event surface weather inventory and ranking polish', function () {
  ...
  assert.ok(/viz-magic-v189/.test(sw), 'service worker should use v112 cache');
});
...
test('v114 loading state does not invent an embercaster level-one character', function () {
  ...
  assert.ok(/viz-magic-v189/.test(swJs), 'service worker should publish v114');
});
...
test('v115 service worker forces stale PWA windows onto the fresh cache-busted app shell', function () {
  ...
  assert.ok(/viz-magic-v189/.test(swJs), 'service worker should publish v115');
});
...
test('production PWA install is available before login and landing icons do not depend on emoji fonts', function () {
  ...
  assert.ok(/viz-magic-v189/.test(swJs), 'service-worker cache should advance for fresh runtime assets');
});
...
test('installed app cold-start has visible fallback and service-worker network timeouts', function () {
  ...
  assert.ok(/viz-magic-v189/.test(swJs), 'service worker cache should advance for cold-start black-screen fix');
...
test('secret maps room renders fifteen daily paid maps and world map markers', function () {
  ...
  assert.ok(ruJs.includes("help_magic_library_chapter_two_intro: 'В бесчисленных тайниках Мира есть множество секретных схронов... Этот первый.'") && ruJs.includes("help_magic_library_chapter_two_warning: 'Отнимает 10% жизненной энергии!!!'"), 'room should preserve its description and separate red warning');
...
test('unknown maps chapter three keeps the selected ten-map order and a separated fading path', function () {
  ...
  assert.ok(/help_magic_library_chapter_three_warning:\s*'Отнимает 10% жизненной энергии!!!'/.test(ruJs), 'chapter three should preserve the exact danger warning');
...
test('middle world maps begin with the separated Revealing Path card', function () {
  ...
  assert.ok(/help_magic_library_middle_warning:\s*'Отнимает 10% жизненной энергии!!!'/.test(ruJs), 'middle maps should preserve the paid red warning');
...
test('verified world maps and font-independent SVG icons remain intact', function () {
  ...
  assert.ok(/viz-magic-v189/.test(swJs), 'service worker cache should advance for text-only library mode');
...
test('world attraction maps keep first masquerade card and Growing Up section', function () {
  ...
  assert.ok(/Карты Притяжения Мира/.test(ruJs) && /help_magic_library_attraction_title/.test(ruJs), 'attraction maps should have a Russian library title');
  ...
  assert.ok(/data-attraction-library-map=\"' \+ firstEntry\.id[\s\S]*help-unknown-library-divider[\s\S]*help_attraction_library_growing[\s\S]*HELP_ATTRACTION_LIBRARY_GROWING_MAPS/.test(helpJs), 'Growing Up title should render under the divider after the first card');
...
test('Magical Library uses the original board-game artwork with lore and Close', function () {
  ...
  assert.ok(/viz-magic-v189/.test(swJs), 'service worker cache should advance for restored board-game artwork');
});
