...
test('magical weather is labelled and affects hunts', function () {
  ...
  assert.ok(/i18n\/ru.js\?v=20260824c/.test(indexHtml), 'Russian weather copy must be cache-busted');
  assert.ok(/i18n\/en.js\?v=20260824c/.test(indexHtml), 'English weather copy must be cache-busted');
  ...
  assert.ok(/main.css\?v=20260823e/.test(indexHtml), 'forecast grid CSS must be cache-busted');
...
test('map regions separate safe travel from gated current-region exploration', function () {
  ...
  assert.ok(/main.css\?v=20260823e/.test(indexHtml), 'map action styles should be cache-busted');
  assert.ok(/i18n\/ru.js\?v=20260824c/.test(indexHtml) && /i18n\/en.js\?v=20260824c/.test(indexHtml), 'map action translations should be cache-busted');
  assert.ok(/viz-magic-v178/.test(swJs), 'service worker cache should advance for the map action layout');
});
...
test('Denis feedback item and motion icons are semantic', function () {
  ...
  assert.ok(/creators-reward-title[\s\S]*section-icon vmagic-breathe/.test(developersJs), 'creator gratitude icon should breathe');
...
test('magical library restores the original board-game artwork with full lore', function () {
  ...
  assert.ok(/main\.css\?v=20260823e/.test(index), 'CSS should be cache-busted for the Magical Library');
  assert.ok(/js\/i18n\/ru\.js\?v=20260824c/.test(index) && /js\/i18n\/en\.js\?v=20260824c/.test(index), 'i18n should be cache-busted for the Magical Library');
  assert.ok(/viz-magic-v178/.test(sw), 'service worker should publish the restored library cache bump');
});
...
test('magical guide replaces extra magical pages tab without shuffling practical help', function () {
  ...
  assert.ok(/main\.css\?v=20260823e/.test(indexHtml), 'main CSS should be cache-busted for guide redesign');
  assert.ok(/viz-magic-v178/.test(swJsV83), 'service worker should use the current v91 cache');
...
test('Denis v91 polish batch keeps quests fair and icons lively', () => {
  ...
  assert.ok(/main\.css\?v=20260823e/.test(indexHtml), 'main CSS should be cache-busted for v91 polish');
  ...
  assert.ok(/viz-magic-v178/.test(swJs), 'service worker should use v91 cache');
});
...
test('profile avatars from VIZ json_metadata are bounded and optional', function () {
  ...
  assert.ok(/main\.css\?v=20260823e/.test(indexHtml), 'avatar CSS should be cache-busted');
  assert.ok(/viz-magic-v178/.test(swJs), 'service worker should use v92 cache');
});
...
test('safe avatar upload UI re-encodes before JSON_METADATA writes', function () {
  ...
  assert.ok(/js\/i18n\/ru\.js\?v=20260824c/.test(indexHtml) && /js\/i18n\/en\.js\?v=20260824c/.test(indexHtml), 'i18n should be cache-busted for avatar strings');
});
...
test('character profile refreshes avatar from VIZ JSON_METADATA on render', function () {
  ...
  assert.ok(/js\/i18n\/ru\.js\?v=20260824c/.test(indexHtmlAvatarProfile) && /js\/i18n\/en\.js\?v=20260824c/.test(indexHtmlAvatarProfile), 'i18n should be cache-busted for updated avatar copy');
});
...
test('Denis v97 world, inventory, nav, arena and guide polish is explicit', function () {
  ...
  assert.ok(/world-events\.js\?v=20260826k/.test(indexV97) && /home\.js\?v=20260826u/.test(indexV97) && /main\.css\?v=20260823e/.test(indexV97), 'v97 files should be cache-busted');
});
...
test('bottom navigation keeps icons above single-line labels and cache-busts changed assets', function () {
  ...
  assert.ok(/main\.css\?v=20260823e/.test(indexHtml), 'main.css cache bust missing');
  ...
  assert.ok(/viz-magic-v178/.test(read('app/sw.js')), 'service worker cache should be v103');
});
...
test('v103 weave surge, default avatars, and guide copy polish are explicit', function () {
  ...
  assert.ok(/main\.css\?v=20260823e/.test(indexHtml), 'v103 CSS should be cache-busted');
  ...
  assert.ok(/viz-magic-v178/.test(swJs), 'service worker should use v103 cache');
});
...
test('quest abandon charges only unstarted quests and warns before forfeit', function () {
  ...
  assert.ok(/js\/i18n\/ru\.js\?v=20260824c/.test(indexPenalty) && /js\/i18n\/en\.js\?v=20260824c/.test(indexPenalty), 'i18n cache bust missing for abandon penalty');
  assert.ok(/viz-magic-v178/.test(swPenalty), 'service worker should use v104 cache');
});
...
test('v110 player feedback keeps requested icons, copy, vital explainers, and event text placement explicit', function () {
  ...
  ['main.css','ru.js','en.js','world-events.js','progress-bar.js','nav.js','home.js','character.js','hunt.js','inventory.js','arena.js','settings.js','leaderboard.js'].forEach(function(asset) {
    var version = asset === 'main.css' ? '20260823e' : ((asset === 'home.js' || asset === 'character.js' || asset === 'nav.js' || asset === 'hunt.js') ? '20260826u' : ((asset === 'ru.js' || asset === 'en.js') ? '20260824c' : '20260826k'));
    assert.ok(new RegExp(asset.replace('.', '\\.') + '\\?v=' + version).test(index), asset + ' should be cache-busted for v110');
  });
  assert.ok(/viz-magic-v178/.test(sw), 'service worker should use v110 cache');
});
...
test('v109 player polish batch removes stale text, fixes motion/copy/icons, and fills hunt tier gap', function () {
  ...
  assert.ok(/viz-magic-v178/.test(swV109), 'service worker should use v109 cache');
});
...
test('v108 inventory, bottom nav, hunt danger marker, and arena motion polish are explicit', function () {
  ...
  assert.ok(/main\.css\?v=20260823e/.test(indexV108), 'main CSS cache bust missing for v108');
  ...
  assert.ok(/js\/i18n\/ru\.js\?v=20260824c/.test(indexV108) && /js\/i18n\/en\.js\?v=20260824c/.test(indexV108), 'i18n cache bust missing for v108 item labels');
  assert.ok(/viz-magic-v178/.test(swV108), 'service worker should use v108 cache');
});
...
test('v107 hunt combat uses spell mana cost, not full account energy', function () {
  ...
  assert.ok(/viz-magic-v178/.test(swV107), 'service worker should use v107 cache');
});
...
test('v106 hunt danger copy is truthful and Weave title is single-line', function () {
  ...
  assert.ok(/main\.css\?v=20260823e/.test(indexV106), 'v106 CSS cache bust missing');
  ...
  assert.ok(/viz-magic-v178/.test(swV106), 'service worker should use v106 cache');
});
...
test('v105 text quality, season colors, avatar title, and spell modal polish are explicit', function () {
  ...
  assert.ok(/main\.css\?v=20260823e/.test(indexV105), 'main CSS cache bust missing for v105');
  ...
  assert.ok(/js\/i18n\/ru\.js\?v=20260824c/.test(indexV105) && /js\/i18n\/en\.js\?v=20260824c/.test(indexV105), 'i18n cache bust missing for v105');
  assert.ok(/viz-magic-v178/.test(swV105), 'service worker should use v105 cache');
});
...
test('v112 player feedback fixes event surface weather inventory and ranking polish', function () {
  ...
  assert.ok(/viz-magic-v178/.test(sw), 'service worker should use v112 cache');
});
...
test('v114 loading state does not invent an embercaster level-one character', function () {
  ...
  assert.ok(/viz-magic-v178/.test(swJs), 'service worker should publish v114');
});
...
test('v115 service worker forces stale PWA windows onto the fresh cache-busted app shell', function () {
  ...
  assert.ok(/viz-magic-v178/.test(swJs), 'service worker should publish v115');
});
...
test('production PWA install is available before login and landing icons do not depend on emoji fonts', function () {
  ...
  assert.ok(/viz-magic-v178/.test(swJs), 'service-worker cache should advance for fresh runtime assets');
});
...
test('installed app cold-start has visible fallback and service-worker network timeouts', function () {
  ...
  assert.ok(/viz-magic-v178/.test(swJs), 'service worker cache should advance for cold-start black-screen fix');
...
test('critical desktop icons avoid emoji glyph squares on Home and bottom nav', function () {
  ...
  assert.ok(/main\.css\?v=20260823e/.test(indexHtml), 'CSS must be cache-busted for icon replacement');
...
test('Creators book replaces the visible Developers section without breaking its reward path', function () {
  ...
  assert.ok(/VizBroadcast\.award\(DEVELOPER_ACCOUNT/.test(developersJs), 'existing voluntary VIZ reward path should remain intact');
...
test('magical library links stay on one line without changing map images', function () {
  assert.ok(/main.css\?v=20260823e/.test(indexHtml), 'CSS must be cache-busted for Magical Library link sizing');
...
test('verified world maps and font-independent SVG icons remain intact', function () {
  ...
  assert.ok(/viz-magic-v178/.test(swJs), 'service worker cache should advance for text-only library mode');
  assert.ok(/main\.css\?v=20260823e/.test(indexHtml), 'SVG icon styles should stay cache-busted');
});
...
test('Magical Library uses the original board-game artwork with lore and Close', function () {
  ...
  assert.ok(/viz-magic-v178/.test(swJs), 'service worker cache should advance for restored board-game artwork');
});
