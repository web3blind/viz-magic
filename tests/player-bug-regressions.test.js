...
const regionsJs = read('app/js/data/regions.js');
const creaturesJs = read('app/js/data/creatures.js');
...
test('hunt and map UX fixes prevent known loops and confusing copy', function () {
  ...
  assert.ok(/hunt_choose_creature:\s*'Выбери существо для охоты'/.test(ruJs), 'Hunt target heading should say creature, not loot');
  assert.ok(!/hunt_choose_creature:\s*'Выбери добычу'/.test(ruJs), 'Hunt target heading should not call the target loot');
  assert.ok(/help_hunt_text:[\s\S]*перечислены имена магических существ[\s\S]*это цели охоты\.[\s\S]*Выбери цель и заклинание/.test(ruJs), 'Hunt guide should say the creature names are hunt goals and use concise target wording');
  assert.ok(!/а не список будущей добычи|существо-цель/.test(ruJs), 'Hunt guide should not keep the removed loot-list or creature-target phrasing');
  assert.ok(/hunt_creature_level_aria:\s*'уровни \{min\}-\{max\}'/.test(ruJs), 'Hunt creature aria-label should read levels in Russian');
  assert.ok(/t\('hunt_creature_level_aria', \{ min: c\.minLevel, max: c\.maxLevel \}\)/.test(huntJs), 'Hunt creature cards should localize level aria-label text');
  assert.ok(/icon:\s*'🔥'/.test(creaturesJs) && /icon:\s*'🌑'/.test(creaturesJs) && /icon:\s*'🌿'/.test(creaturesJs), 'starter hunt creatures should have individual visible icons');
  assert.ok(/icon:\s*'🛡️'/.test(creaturesJs) && /icon:\s*'👾'/.test(creaturesJs) && /icon:\s*'🌀'/.test(creaturesJs) && /icon:\s*'👁️'/.test(creaturesJs) && /icon:\s*'🗿'/.test(creaturesJs), 'advanced hunt creatures should have individual visible icons');
  assert.ok(/creature-icon vmagic-breathe/.test(huntJs), 'creature icons should breathe with the global world-motion setting');
  assert.ok(/aria-hidden="true">' \+ Helpers\.escapeHtml\(c\.icon \|\| '✦'\)/.test(huntJs), 'creature icons should be decorative and omitted from screen-reader labels');
  assert.ok(/\.creature-main/.test(mainCss) && /\.creature-icon/.test(mainCss), 'creature icon layout should have explicit CSS');
  assert.ok(/HUNT_POWER_OPTIONS = \[[\s\S]*energy: 100[\s\S]*energy: 300[\s\S]*energy: 500[\s\S]*energy: 700/.test(huntJs), 'hunt should expose four ordinary power choices: 1%, 3%, 5%, and 7%');
  assert.ok(/hunt-power-grid[\s\S]*role="radiogroup"/.test(huntJs), 'hunt power choices should be a keyboard-accessible radio group');
  assert.ok(/selectedHuntEnergy = Number\(option\.getAttribute\('data-energy'\)\)/.test(huntJs), 'hunt power radio buttons should update the selected energy');
  assert.ok(/var huntEnergy = _getSelectedHuntEnergy\(spell\)/.test(huntJs) && /VizBroadcast\.huntAction[\s\S]*huntEnergy/.test(huntJs), 'live hunt should broadcast the selected power energy');
  assert.ok(/_resolveHuntFromBlock\(blockNum, ch, creature, spell, huntEnergy/.test(huntJs), 'live hunt should resolve combat with selected power energy');
  assert.ok(/var combatEnergy = data\.energy \|\| spell\.manaCost/.test(stateEngineJs), 'replay should resolve combat with on-chain hunt power energy');
  assert.ok(/allowedEnergy = \[100, 300, 500, 700, spell\.manaCost/.test(read('app/js/engine/validator.js')), 'validator should allow only the ordinary hunt power choices or the spell native cost');
  assert.ok(/maxRounds = playerEnergy <= 700 \? 45 : 25/.test(combatJs), '1%, 3%, 5%, and 7% hunt power should all keep patient combat rounds');
  assert.ok(/hunt_choose_spell_power/.test(huntJs + ruJs + enJs) && /Выбери заклинание и силу охоты/.test(ruJs), 'hunt heading should combine spell and hunt power');
  assert.ok(/hunt_power_hint:\s*'Больше энергии - сильнее удар и выше шанс победить\.'/.test(ruJs), 'hunt should show the requested short power hint');
  assert.ok(/_getDefaultHuntSpellId/.test(huntJs) && !/spell-grid\" role=\"radiogroup/.test(huntJs), 'old visible spell-cost buttons should be removed while a default spell remains for protocol compatibility');
  assert.ok(!/hunt_min_mana_hint|hunt_choose_power:|Награда остаётся честной|XP и добыча|hunt_power_.*_desc|hunt-power-desc|spell-btn|spell-grid|spell-cost|spell-warning/.test(huntJs + ruJs + enJs + mainCss), 'old spell buttons, duplicate power heading, reward caveat, and per-button descriptions should stay removed');
  assert.ok(/\.hunt-power-grid/.test(mainCss) && /repeat\(2, minmax\(0, 1fr\)\)/.test(mainCss) && /@media \(min-width: 720px\)[\s\S]*repeat\(4, minmax\(0, 1fr\)\)/.test(mainCss), 'hunt power layout should fit narrow mobile and desktop');
  assert.ok(/css\/accessibility\.css\?v=20260902b/.test(indexHtml), 'accessibility CSS should be cache-busted after removing old spell button focus styles');
...
test('large stale checkpoint catch-up uses archive events instead of replaying empty blocks', function () {
  ...
  assert.ok(/_archiveBlockNeedsHydration/.test(appJs) && /HistorySource\.getProofBlock \|\| HistorySource\.getBlock/.test(appJs) && /BlockProcessor\.processBlock\(fullBlock, proofBlockNum\)/.test(appJs), 'incomplete paid proof blocks should hydrate a complete authoritative game-operation set');
...
test('magical weather is labelled and affects hunts', function () {
  ...
  assert.ok(/i18n\/ru.js\?v=20260824f/.test(indexHtml), 'Russian weather copy must be cache-busted');
  assert.ok(/i18n\/en.js\?v=20260824f/.test(indexHtml), 'English weather copy must be cache-busted');
...
test('hunt screen exposes explicit camp rest promised by Help', function () {
  assert.ok(/hunt.js\?v=20260826aa/.test(indexHtml), 'hunt screen should be cache-busted');
  ...
  assert.ok(/Отдых у костра/.test(ruJs) && /нажми <button type="button" class="help-nav-link" data-help-nav="hunt">Отдых у костра<\/button>/.test(ruJs), 'Russian Help should keep only the visible rest button without duplicating its label before the button');
  assert.ok(!/нажми «Отдых у костра» <button/.test(ruJs), 'Russian Help should not duplicate the camp-rest phrase before the button');
});
...
test('minor rift banner explains itself and is actionable', function () {
  ...
  assert.ok(/\.event-banner-minor_rift \.event-name \{ color: #7dd3fc;/.test(mainCss), 'minor rift title should have its own distinct color');
...
test('hunt rest uses home-scale HP values', function () {
  assert.ok(/hunt.js\?v=20260826aa/.test(indexHtml), 'hunt screen should be cache-busted for HP display scale');
...
test('hunt headings and help use updated thematic icons', function () {
  assert.ok(/hunt.js\?v=20260826aa/.test(indexHtml), 'hunt screen should be cache-busted for heading icons');
  ...
  assert.ok(/help.js\?v=20260824c/.test(indexHtml), 'help screen should be cache-busted for hunt icon');
...
test('map regions separate safe travel from gated current-region exploration', function () {
  ...
  assert.ok(/js\/ui\/screens\/map.js\?v=20260824h/.test(indexHtml), 'map controls should be cache-busted');
  ...
  assert.ok(/viz-magic-v218/.test(swJs), 'service worker cache should advance for the map action layout');
});
...
test('map travel hint links to a permanent travel and exploration guide page', function () {
  ...
  assert.ok((ruJs.match(/data-help-nav="crafting">Мастерской<\/button>/g) || []).length === 1, 'only the introductory Workshop mention should stay an active link');
  assert.ok(/data-help-nav="crafting">Мастерской<\/button><br>0,10% энергии/.test(ruJs), 'the introductory Workshop button should not be followed by a dot');
  assert.ok(!/data-help-nav="crafting">Мастерской<\/button>\.<br>0,10% энергии/.test(ruJs), 'the dot after the Workshop button should be removed');
  assert.ok(/шанс попутной находки для мастерской во время путешествия/.test(ruJs), 'the chance-table Workshop mention should be plain text');
...
test('magical guide explains versioned XP thresholds without claiming a 100-percent mana level-up', function () {
  assert.ok(/\{ key: 'levels',\s*icon: 'xp' \}/.test(helpJs), 'the guide should contain a dedicated Levels and XP page');
  assert.ok(/help_section_levels:\s*'Уровни и опыт'/.test(ruJs), 'the Russian guide should name the levels page');
  assert.ok(/1-2 — 1000 XP<br>2-3 — 1250 XP<br>3-4 — 1750 XP<br>4-5 — 2500 XP<br>5-6 — 3500 XP<br>6-7 — 4750 XP<br>7-8 — 6250 XP<br>8-9 — 8000 XP<br>9-10 — 10000 XP/.test(ruJs), 'the guide should publish approved thresholds one transition per line');
  assert.ok(!/С блока 83 000 000|Достигнутый уровень и уже заработанная часть следующего уровня сохраняются|Старые действия до блока активации|Суммарно потраченные 100% Маны сами по себе не повышают уровень/.test(ruJs), 'the levels page should remove the old activation and migration explanation');
  assert.ok(/Текущий прогресс XP показан на страницах <button type="button" class="help-nav-link" data-help-nav="home">Дом<\/button> и <button type="button" class="help-nav-link" data-help-nav="character">Персонаж<\/button>[^.]/.test(ruJs), 'Home and Character should be keyboard-safe navigation links without a dot after Character');
  assert.ok(/Охота, Армагеддон, завершённые дуэли и задания дают XP/.test(ruJs), 'the guide should name the implemented XP sources');
});
...
test('summer weather copy and repeated lore tails stay deduplicated', function () {
  ...
  assert.ok(/weather_dynamic_creature: 'Сила существ'/.test(ruJs), 'creature strength label should not call hunt targets loot in Russian');
...
test('Denis feedback item and motion icons are semantic', function () {
  ...
  assert.ok(/REGION_DISPLAY_LEVEL_RANGES = \{[\s\S]*commons_first_light: \{ min: 1, max: 7 \}[\s\S]*void_sanctum: \{ min: 99, max: 105 \}/.test(regionsJs), 'world-map home-card level ranges should advance by seven in scroll order');
  assert.ok(/GameRegions\.getDisplayLevelRange\(regionId\)/.test(mapJs), 'world-map cards and lore modal should use shared display ranges');
  assert.ok(!/region-home-badge|region-home-level|map_home_card|Ваша карта|Your map/.test(mapJs + mainCss + ruJs + enJs), 'the extra home-map badge should be removed; the current card is marked only by the status text');
  assert.ok(/map_you_are_here:\s*'Ты здесь'/.test(ruJs) && /map_current:\s*'Ты здесь'/.test(ruJs), 'Russian current-map statuses should use informal Ты здесь');
  assert.ok(!/map_level_too_low|map_locked_until_level|map_locked_until_level_short|disabled aria-disabled="true"|Helpers\.icon\('lock', 'travel-icon'\)|GameRegions\.getDisplayLevelMin\(regionId\)/.test(mapJs + ruJs + enJs), 'world-map travel buttons should never show or enforce level locks');
  assert.ok(/Helpers\.icon\('map', 'travel-icon'\) \+ ' ' \+ Helpers\.manaCost\(TRAVEL_COST_LOW\)/.test(mapJs), 'all world-map travel buttons should show the energy cost');
...
test('Denis v70 polish keeps motion icons, honest low-mana hunt, and chronicle names clean', function () {
  ...
  assert.ok(/_getDefaultHuntSpellId[\s\S]*MIN_HUNT_COST/.test(huntJs) && /spell\.manaCost[\s\S]*MIN_HUNT_COST/.test(huntJs) && /hunt_spell_too_weak/.test(huntJs + ruJs + enJs), 'hunt should auto-pick a usable spell and still guard against any combat spell below the 1% minimum');
  ...
  assert.ok(/maxRounds = playerEnergy <= 700 \? 45 : 25/.test(read('app/js/engine/combat.js')), '1%, 3%, 5%, and 7% economical hunts should all allow longer fights');
...
test('v82 Denis feedback polish is explicit and cache-busted', function () {
  ...
  assert.ok(/hunt\.js\?v=20260826aa/.test(indexHtml), 'Hunt should be cache-busted for Armageddon lock feedback');
...
test('magical library restores the original board-game artwork with full lore', function () {
  ...
  assert.ok(/viz-magic-v218/.test(sw), 'service worker should publish the restored library cache bump');
});
...
test('magical guide replaces extra magical pages tab without shuffling practical help', function () {
  ...
  assert.ok(/viz-magic-v218/.test(swJsV83), 'service worker should use the current v91 cache');
...
test('Denis v91 polish batch keeps quests fair and icons lively', () => {
  ...
  assert.ok(/viz-magic-v218/.test(swJs), 'service worker should use v91 cache');
});
...
test('profile avatars from VIZ json_metadata are bounded and optional', function () {
  ...
  assert.ok(/viz-magic-v218/.test(swJs), 'service worker should use v92 cache');
});
...
test('safe avatar upload UI re-encodes before JSON_METADATA writes', function () {
  ...
  assert.ok(/js\/i18n\/ru\.js\?v=20260824f/.test(indexHtml) && /js\/i18n\/en\.js\?v=20260824f/.test(indexHtml), 'i18n should be cache-busted for avatar strings');
});
...
test('character profile refreshes avatar from VIZ JSON_METADATA on render', function () {
  ...
  assert.ok(/js\/i18n\/ru\.js\?v=20260824f/.test(indexHtmlAvatarProfile) && /js\/i18n\/en\.js\?v=20260824f/.test(indexHtmlAvatarProfile), 'i18n should be cache-busted for updated avatar copy');
});
...
test('bottom navigation keeps icons above single-line labels and cache-busts changed assets', function () {
  ...
  assert.ok(/hunt\.js\?v=20260826aa/.test(indexHtml), 'hunt cache bust missing');
  ...
  assert.ok(/viz-magic-v218/.test(read('app/sw.js')), 'service worker cache should be v103');
});
...
test('v103 weave surge, default avatars, and guide copy polish are explicit', function () {
  ...
  assert.ok(/data-help-nav=\"character\">Персонаж<\/button>[^.]/.test(ruJs), 'Levels guide should not show a dot after the Character nav button');
  assert.ok(/data-help-nav=\"quests\">Задания<\/button>[^.]/.test(ruJs), 'Quests guide should not show a dot after the Quests nav button');
  assert.ok(/data-help-nav=\"crafting\">Мастерской<\/button><br>0,10% энергии/.test(ruJs), 'Travel guide should remove the empty line after the Workshop nav button');
  assert.ok(/help_armageddon_text:[\s\S]*Дроп: Thornvine[^']*\.<br>Крафт: Echo Shards/.test(ruJs), 'Armageddon Craft sentence should begin on the next line');
  assert.ok(/data-help-nav=\"crafting\">Мастерскую<\/button> выберите рецепт/.test(ruJs), 'Workshop guide should remove the comma after the Workshop nav button');
  assert.ok(/data-help-nav=\"settings\">Настройках<\/button> Также воспроизводит/.test(ruJs), 'Battle Narrator guide should remove the dot after the Settings nav button');
  assert.ok(/help_magic_ranks_text:[\s\S]*науке:<br>1-10 Искатель,<br>11-20 Адепт,<br>21-30 Знающий,<br>31-40 Заклинатель/.test(ruJs), 'Magic ranks should start each level range on a new line');
  ...
  assert.ok(/viz-magic-v218/.test(swJs), 'service worker should publish the latest guide copy cache');
  ...
  assert.ok(/viz-magic-v218/.test(swJs), 'service worker should use v103 cache');
});
...
test('quest abandon charges only unstarted quests and warns before forfeit', function () {
  ...
  assert.ok(/viz-magic-v218/.test(swPenalty), 'service worker should use v104 cache');
});
...
test('v110 player feedback keeps requested icons, copy, vital explainers, and event text placement explicit', function () {
  ...
  assert.ok(/viz-magic-v218/.test(sw), 'service worker should use v110 cache');
});
...
test('v109 player polish batch removes stale text, fixes motion/copy/icons, and fills hunt tier gap', function () {
  ...
  assert.ok(/viz-magic-v218/.test(swV109), 'service worker should use v109 cache');
});
...
test('v108 inventory, bottom nav, hunt danger marker, and arena motion polish are explicit', function () {
  ...
  assert.ok(/hunt\.js\?v=20260826aa/.test(indexV108), 'hunt cache bust missing for v108');
  ...
  assert.ok(/viz-magic-v218/.test(swV108), 'service worker should use v108 cache');
});
...
test('v107 hunt combat uses spell mana cost, not full account energy', function () {
  ...
  assert.ok(/validator\.js\?v=20260731d/.test(indexV107), 'validator cache bust missing for v107 hunt energy fix');
  ...
  assert.ok(/viz-magic-v218/.test(swV107), 'service worker should use v107 cache');
});
...
test('v106 hunt danger copy is truthful and Weave title is single-line', function () {
  ...
  assert.ok(/viz-magic-v218/.test(swV106), 'service worker should use v106 cache');
});
...
test('v105 text quality, season colors, avatar title, and spell modal polish are explicit', function () {
  ...
  assert.ok(/viz-magic-v218/.test(swV105), 'service worker should use v105 cache');
});
...
test('v112 player feedback fixes event surface weather inventory and ranking polish', function () {
  ...
  assert.ok(/viz-magic-v218/.test(sw), 'service worker should use v112 cache');
});
...
test('v114 loading state does not invent an embercaster level-one character', function () {
  ...
  assert.ok(/viz-magic-v218/.test(swJs), 'service worker should publish v114');
});
...
test('v115 service worker forces stale PWA windows onto the fresh cache-busted app shell', function () {
  ...
  assert.ok(/viz-magic-v218/.test(swJs), 'service worker should publish v115');
});
...
test('production PWA install is available before login and landing icons do not depend on emoji fonts', function () {
  ...
  assert.ok(/viz-magic-v218/.test(swJs), 'service-worker cache should advance for fresh runtime assets');
});
...
test('installed app cold-start has visible fallback and service-worker network timeouts', function () {
  ...
  assert.ok(/viz-magic-v218/.test(swJs), 'service worker cache should advance for cold-start black-screen fix');
...
test('secret maps room requires an exact daily award and unlock action in one transaction', function () {
  ...
  const chapterFiveProofBlock = {
    vmActions: [{ sender: 'alice', txIndex: 11, action: { type: 'library.unlock', data: { chapter: 'chapter5', day: day } } }],
    awards: [{ initiator: 'alice', receiver: 'denis-skripnik', energy: 1000, memo: 'viz://vm/library/chapter5/' + day, txIndex: 11 }]
  };
  ...
  assert.strictEqual(engine.verifyLibraryUnlockProof(chapterFourProofBlock, 'alice', 'chapter5', day), false, 'chapter four payment must not open chapter five');
  assert.strictEqual(engine.verifyLibraryUnlockProof(chapterFiveProofBlock, 'alice', 'chapter5', day), true, 'chapter five should verify its own exact atomic proof');
  assert.strictEqual(engine.verifyLibraryUnlockProof(chapterFiveProofBlock, 'alice', 'chapter2', day), false, 'chapter five payment must not open chapter two');
  assert.strictEqual(engine.verifyLibraryUnlockProof(chapterFiveProofBlock, 'alice', 'chapter3', day), false, 'chapter five payment must not open chapter three');
  assert.strictEqual(engine.verifyLibraryUnlockProof(chapterFiveProofBlock, 'alice', 'chapter4', day), false, 'chapter five payment must not open chapter four');
  ...
  assert.strictEqual(engine.verifyLibraryUnlockProof(normalizedProofBlock, 'alice', 'chapter5', day), false, 'chapter two payment must not open chapter five');
  ...
  engine.processBlock({
    ...
    vmActions: [
      ...
      unlock('chapter-five', 11, day, 'chapter5'),
      unlock('cross-attraction', 12, day, 'chapter5')
    ],
    voicePosts: [],
    awards: [
      ...
      { initiator: 'chapter-five', receiver: 'denis-skripnik', energy: 1000, memo: 'viz://vm/library/chapter5/' + day, txIndex: 11 },
      { initiator: 'cross-attraction', receiver: 'denis-skripnik', energy: 1000, memo: 'viz://vm/library/chapter4/' + day, txIndex: 12 }
    ]
  });
  ...
  assert.strictEqual(engine.hasLibraryAccess('chapter-five', 'chapter5', day), true, 'exact chapter five replay should open attraction maps');
  assert.strictEqual(engine.hasLibraryAccess('chapter-five', 'chapter2', day), false, 'chapter five replay should not open chapter two');
  assert.strictEqual(engine.hasLibraryAccess('chapter-five', 'chapter3', day), false, 'chapter five replay should not open chapter three');
  assert.strictEqual(engine.hasLibraryAccess('chapter-five', 'chapter4', day), false, 'chapter five replay should not open chapter four');
  assert.strictEqual(engine.hasLibraryAccess('cross-attraction', 'chapter5', day), false, 'chapter four memo must not validate a chapter five action');
...
test('middle world maps begin with the separated Revealing Path card', function () {
  assert.ok(ruJs.includes("help_middle_library_revealing_path: 'Проявляющийся Путь'"), 'middle maps should name Denis selected first group');
  var revealingMapSource = helpJs.match(/var HELP_MIDDLE_LIBRARY_REVEALING_MAPS\s*=\s*\[([\s\S]*?)\];/);
  var middleMapSource = helpJs.match(/var HELP_MIDDLE_LIBRARY_MAPS\s*=\s*\[([\s\S]*?)\];/);
  assert.ok(revealingMapSource && middleMapSource, 'middle maps should keep explicit separated lists');
  assert.deepStrictEqual(Array.from(revealingMapSource[1].matchAll(/id:\s*'(\d+)'/g), function (match) { return match[1]; }), ['14'], 'Saffron Saltstorm Caravan should be the first separated Revealing Path card');
  assert.strictEqual(Array.from(middleMapSource[1].matchAll(/id:\s*'(14)'/g)).length, 0, 'the rest of the middle maps list should not duplicate card 14');
...
test('secret maps daily broadcast builder binds award and proof to the same day', function () {
  ...
  sent = null;
  sent = null;
  context.VizBroadcast.libraryUnlockChapterAction('chapter5', 1000, '2026-08-23', function (err) { error = err; });
  assert.ifError(error);
...
test('verified world maps and font-independent SVG icons remain intact', function () {
  ...
  assert.ok(/viz-magic-v218/.test(swJs), 'service worker cache should advance for text-only library mode');
...
test('world attraction maps keep first masquerade card and Growing Up section', function () {
  ...
  assert.ok(firstMapSource && growingMapSource, 'attraction maps should keep explicit first and growing lists');
  ...
  assert.ok(/help-attraction-library-status[\s\S]*attractionLibraryBusy \? t\('help_secret_library_checking'\)/.test(helpJs), 'attraction unlock should announce checking immediately and after rerender');
  assert.ok(/var attractionBusyAttrs = attractionLibraryBusy \? ' disabled aria-disabled="true" aria-busy="true"'/.test(helpJs), 'attraction unlock button should stay disabled and busy after rerender');
  assert.ok(/_setAttractionLibraryStatus\(Helpers\.t\('help_secret_library_checking'\)\)/.test(helpJs), 'attraction unlock should set live status on the first click before broadcast');
  assert.ok(/if \(attractionLibraryBusy\) return;/.test(helpJs), 'attraction unlock should ignore repeat clicks while a payment is in progress');
  assert.ok(/button\.removeAttribute\('aria-disabled'\)/.test(helpJs), 'failed attraction unlock should restore aria-disabled with the button state');
  assert.ok(/data-attraction-library-map=\"' \+ firstEntry\.id[\s\S]*help-unknown-library-divider[\s\S]*help_attraction_library_growing[\s\S]*HELP_ATTRACTION_LIBRARY_GROWING_MAPS/.test(helpJs), 'Growing Up title should render under the divider after the first card once unlocked');
  assert.ok(/_bindAttractionLibrary/.test(helpJs) && /_findAttractionLibraryEntry/.test(helpJs), 'attraction links should have their own keyboard-safe click binding and lookup');
  assert.ok(/HELP_ATTRACTION_LIBRARY_FIRST_MAPS\.concat\(HELP_ATTRACTION_LIBRARY_GROWING_MAPS\)/.test(helpJs), 'attraction modal lookup should include the separated first card');
...
test('Magical Library uses the original board-game artwork with lore and Close', function () {
  ...
  assert.ok(/viz-magic-v218/.test(swJs), 'service worker cache should advance for restored board-game artwork');
});
...
test('player name uses the chosen rune plaque on Home and Character', function () {
  ...
  assert.ok(/home-welcome-text/.test(homeJs), 'Home should expose the welcome text for its matching gold color');
  assert.ok(/player-name-rune/.test(homeJs) && /player-name-rune/.test(characterJs), 'Home and Character should share the chosen rune plaque');
  assert.ok(/player-name-rune-spark[^>]*aria-hidden="true"/.test(homeJs) && /player-name-rune-spark[^>]*aria-hidden="true"/.test(characterJs), 'decorative rune sparks should stay hidden from screen readers');
  assert.ok(/\.home-welcome-text\s*\{[\s\S]*color:\s*#ffe7a3/.test(mainCss), 'welcome text should use the plaque gold');
  assert.ok(/\.player-name-rune\s*\{[\s\S]*background:\s*linear-gradient\(135deg,\s*rgba\(93,\s*61,\s*29,\s*0\.96\),\s*rgba\(27,\s*22,\s*42,\s*0\.98\)\)/.test(mainCss), 'rune plaque should keep the approved bronze-violet surface');
  ...
  assert.ok(/viz-magic-v218/.test(swJs), 'service worker cache should advance for the rune plaque');
});
