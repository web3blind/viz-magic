...
test('magical weather is labelled and affects hunts', function () {
  ...
  assert.ok(/i18n\/ru.js\?v=20260822p/.test(indexHtml), 'Russian weather copy must be cache-busted');
  assert.ok(/i18n\/en.js\?v=20260822p/.test(indexHtml), 'English weather copy must be cache-busted');
  ...
  assert.ok(/main.css\?v=20260822f/.test(indexHtml), 'forecast grid CSS must be cache-busted');
...
test('desktop standalone map zoom uses fullscreen overlay and robust emoji fallback', function () {
  assert.ok(/MAP_ASSET_VERSION = '20260826u'/.test(mapScreenJs), 'map images should be cache-busted after fullscreen zoom change');
...
test('hunt screen exposes explicit camp rest promised by Help', function () {
  ...
  assert.ok(/broadcast.js\?v=20260822h/.test(indexHtml), 'broadcast helper should be cache-busted for restAction');
  assert.ok(/state-engine.js\?v=20260822n/.test(indexHtml), 'state-engine should be cache-busted for processRestResult');
...
test('hunt headings and help use updated thematic icons', function () {
  ...
  assert.ok(/help.js\?v=20260822o/.test(indexHtml), 'help screen should be cache-busted for hunt icon');
...
test('world boss UI can enter active window from schedule even without spawn checkpoint', function () {
  ...
  assert.ok(/state-engine.js\?v=20260822n/.test(indexHtml), 'state engine should be cache-busted for boss attack spawn-block parity');
...
test('map regions separate safe travel from gated current-region exploration', function () {
  ...
  assert.ok(/js\/ui\/screens\/map.js\?v=20260826x/.test(indexHtml), 'map controls should be cache-busted');
  assert.ok(/main.css\?v=20260822f/.test(indexHtml), 'map action styles should be cache-busted');
  assert.ok(/i18n\/ru.js\?v=20260822p/.test(indexHtml) && /i18n\/en.js\?v=20260822p/.test(indexHtml), 'map action translations should be cache-busted');
  assert.ok(/viz-magic-v168/.test(swJs), 'service worker cache should advance for the map action layout');
});
...
test('map regions are lore links with travel buttons everywhere', function () {
  ...
  assert.ok(/MAP_ASSET_VERSION = '20260826u'/.test(mapJs), 'map image URLs should use the current cache-bust version after portal retouch');
...
test('developers screen offers optional non-advantage award', function () {
  ...
  assert.ok(/app.js\?v=20260822l/.test(indexHtml), 'app controller should be cache-busted for developers route');
...
test('magical library restores the original board-game artwork with full lore', function () {
  ...
  assert.ok(/help\.js\?v=20260822o/.test(index), 'Help should be cache-busted for restored board-game artwork');
  assert.ok(/main\.css\?v=20260822f/.test(index), 'CSS should be cache-busted for the Magical Library');
  assert.ok(/js\/i18n\/ru\.js\?v=20260822p/.test(index) && /js\/i18n\/en\.js\?v=20260822p/.test(index), 'i18n should be cache-busted for the Magical Library');
  assert.ok(/viz-magic-v168/.test(sw), 'service worker should publish the restored library cache bump');
});
...
test('magical guide replaces extra magical pages tab without shuffling practical help', function () {
  ...
  assert.ok(/help\.js\?v=20260822o/.test(indexHtml), 'Help should be cache-busted for guide redesign');
  assert.ok(/main\.css\?v=20260822f/.test(indexHtml), 'main CSS should be cache-busted for guide redesign');
  assert.ok(/viz-magic-v168/.test(swJsV83), 'service worker should use the current v91 cache');
...
test('Denis v91 polish batch keeps quests fair and icons lively', () => {
  ...
  assert.ok(/main\.css\?v=20260822f/.test(indexHtml), 'main CSS should be cache-busted for v91 polish');
  ...
  assert.ok(/viz-magic-v168/.test(swJs), 'service worker should use v91 cache');
});
...
test('profile avatars from VIZ json_metadata are bounded and optional', function () {
  ...
  assert.ok(/main\.css\?v=20260822f/.test(indexHtml), 'avatar CSS should be cache-busted');
  assert.ok(/viz-magic-v168/.test(swJs), 'service worker should use v92 cache');
});
...
test('safe avatar upload UI re-encodes before JSON_METADATA writes', function () {
  ...
  assert.ok(/js\/i18n\/ru\.js\?v=20260822p/.test(indexHtml) && /js\/i18n\/en\.js\?v=20260822p/.test(indexHtml), 'i18n should be cache-busted for avatar strings');
});
...
test('character profile refreshes avatar from VIZ JSON_METADATA on render', function () {
  ...
  assert.ok(/js\/i18n\/ru\.js\?v=20260822p/.test(indexHtmlAvatarProfile) && /js\/i18n\/en\.js\?v=20260822p/.test(indexHtmlAvatarProfile), 'i18n should be cache-busted for updated avatar copy');
});
...
test('Denis v97 world, inventory, nav, arena and guide polish is explicit', function () {
  ...
  assert.ok(/world-events\.js\?v=20260826k/.test(indexV97) && /home\.js\?v=20260826u/.test(indexV97) && /main\.css\?v=20260822f/.test(indexV97), 'v97 files should be cache-busted');
});
...
test('bottom navigation keeps icons above single-line labels and cache-busts changed assets', function () {
  ...
  assert.ok(/main\.css\?v=20260822f/.test(indexHtml), 'main.css cache bust missing');
  ...
  assert.ok(/viz-magic-v168/.test(read('app/sw.js')), 'service worker cache should be v103');
});
...
test('v103 weave surge, default avatars, and guide copy polish are explicit', function () {
  ...
  assert.ok(/main\.css\?v=20260822f/.test(indexHtml), 'v103 CSS should be cache-busted');
  ...
  assert.ok(/settings\.js\?v=20260826k/.test(indexHtml) && /help\.js\?v=20260822o/.test(indexHtml), 'v103 Settings and Help should be cache-busted');
  assert.ok(/viz-magic-v168/.test(swJs), 'service worker should use v103 cache');
});
...
test('quest abandon charges only unstarted quests and warns before forfeit', function () {
  ...
  assert.ok(/state-engine\.js\?v=20260822n/.test(indexPenalty), 'state engine cache bust missing for abandon penalty');
  assert.ok(/quests\.js\?v=20260826k/.test(indexPenalty), 'quests screen cache bust missing for abandon penalty');
  assert.ok(/js\/i18n\/ru\.js\?v=20260822p/.test(indexPenalty) && /js\/i18n\/en\.js\?v=20260822p/.test(indexPenalty), 'i18n cache bust missing for abandon penalty');
  assert.ok(/viz-magic-v168/.test(swPenalty), 'service worker should use v104 cache');
});
...
test('v110 player feedback keeps requested icons, copy, vital explainers, and event text placement explicit', function () {
  ...
  ['main.css','ru.js','en.js','world-events.js','progress-bar.js','nav.js','home.js','character.js','hunt.js','inventory.js','arena.js','settings.js','leaderboard.js'].forEach(function(asset) {
    var version = asset === 'main.css' ? '20260822f' : ((asset === 'home.js' || asset === 'character.js' || asset === 'nav.js' || asset === 'hunt.js') ? '20260826u' : ((asset === 'ru.js' || asset === 'en.js') ? '20260822p' : '20260826k'));
    assert.ok(new RegExp(asset.replace('.', '\\.') + '\\?v=' + version).test(index), asset + ' should be cache-busted for v110');
  });
  assert.ok(/viz-magic-v168/.test(sw), 'service worker should use v110 cache');
});
...
test('v109 player polish batch removes stale text, fixes motion/copy/icons, and fills hunt tier gap', function () {
  ...
  assert.ok(/viz-magic-v168/.test(swV109), 'service worker should use v109 cache');
});
...
test('v108 inventory, bottom nav, hunt danger marker, and arena motion polish are explicit', function () {
  ...
  assert.ok(/main\.css\?v=20260822f/.test(indexV108), 'main CSS cache bust missing for v108');
  ...
  assert.ok(/js\/i18n\/ru\.js\?v=20260822p/.test(indexV108) && /js\/i18n\/en\.js\?v=20260822p/.test(indexV108), 'i18n cache bust missing for v108 item labels');
  assert.ok(/viz-magic-v168/.test(swV108), 'service worker should use v108 cache');
});
...
test('v107 hunt combat uses spell mana cost, not full account energy', function () {
  ...
  assert.ok(/broadcast\.js\?v=20260822h/.test(indexV107), 'broadcast cache bust missing for v107 hunt energy fix');
  ...
  assert.ok(/state-engine\.js\?v=20260822n/.test(indexV107), 'state engine cache bust missing for v107 hunt energy fix');
  assert.ok(/hunt\.js\?v=20260826u/.test(indexV107), 'hunt screen cache bust missing for v107 hunt energy fix');
  assert.ok(/viz-magic-v168/.test(swV107), 'service worker should use v107 cache');
});
...
test('v106 hunt danger copy is truthful and Weave title is single-line', function () {
  ...
  assert.ok(/main\.css\?v=20260822f/.test(indexV106), 'v106 CSS cache bust missing');
  ...
  assert.ok(/viz-magic-v168/.test(swV106), 'service worker should use v106 cache');
});
...
test('v105 text quality, season colors, avatar title, and spell modal polish are explicit', function () {
  ...
  assert.ok(/main\.css\?v=20260822f/.test(indexV105), 'main CSS cache bust missing for v105');
  ...
  assert.ok(/js\/i18n\/ru\.js\?v=20260822p/.test(indexV105) && /js\/i18n\/en\.js\?v=20260822p/.test(indexV105), 'i18n cache bust missing for v105');
  assert.ok(/viz-magic-v168/.test(swV105), 'service worker should use v105 cache');
});
...
test('v112 player feedback fixes event surface weather inventory and ranking polish', function () {
  ...
  assert.ok(/viz-magic-v168/.test(sw), 'service worker should use v112 cache');
});
...
test('v114 loading state does not invent an embercaster level-one character', function () {
  ...
  assert.ok(/viz-magic-v168/.test(swJs), 'service worker should publish v114');
});
...
test('v115 service worker forces stale PWA windows onto the fresh cache-busted app shell', function () {
  ...
  assert.ok(/viz-magic-v168/.test(swJs), 'service worker should publish v115');
});
...
test('production PWA install is available before login and landing icons do not depend on emoji fonts', function () {
  ...
  assert.ok(/viz-magic-v168/.test(swJs), 'service-worker cache should advance for fresh runtime assets');
});
...
test('installed app cold-start has visible fallback and service-worker network timeouts', function () {
  ...
  assert.ok(/viz-magic-v168/.test(swJs), 'service worker cache should advance for cold-start black-screen fix');
...
test('critical desktop icons avoid emoji glyph squares on Home and bottom nav', function () {
  ...
  assert.ok(/main\.css\?v=20260822f/.test(indexHtml), 'CSS must be cache-busted for icon replacement');
...
test('magical library chapter two requires an exact award and unlock action in one transaction', function () {
  const blockContext = {
  ...
  const atomicBlock = blockContext.BlockProcessor.processBlock({
    block_id: 'atomic-library-block', previous: 'previous', timestamp: '2026-08-22T00:00:00',
    transactions: [
      { operations: [] },
      { operations: [
        ['award', { initiator: 'alice', receiver: 'denis-skripnik', energy: 1000, custom_sequence: 0, memo: 'viz://vm/library/chapter2', beneficiaries: [] }],
        ['custom', { id: 'VIZMAGIC', required_regular_auths: ['alice'], required_active_auths: [], json: JSON.stringify({ p: 'VIZMAGIC', t: 'library.unlock', d: { chapter: 'chapter2' } }) }]
      ] }
    ]
  }, 500);
  ...
  const normalizedProofBlock = {
    vmActions: [{ sender: 'alice', txIndex: 1, action: { type: 'library.unlock', data: { chapter: 'chapter2' } } }],
    awards: atomicBlock.awards
  };
  assert.strictEqual(engine.verifyLibraryUnlockProof(normalizedProofBlock, 'alice', 'chapter2'), true, 'full atomic proof block should verify before restoring an old entitlement');
  assert.strictEqual(engine.verifyLibraryUnlockProof({ vmActions: normalizedProofBlock.vmActions, awards: [] }, 'alice', 'chapter2'), false, 'custom action without its award must not pass entitlement preflight');
  const unlock = function(account, txIndex) {
    return { sender: account, txIndex: txIndex, action: { type: 'library.unlock', data: { chapter: 'chapter2' } } };
  };
  ...
  engine.processBlock({
    ...
    vmActions: [
      unlock('alice', 1),
      unlock('alice', 1),
      unlock('action-only', 3),
      unlock('mismatched-tx', 4),
      unlock('wrong-memo', 6)
    ],
    voicePosts: [],
    awards: [
      { initiator: 'alice', receiver: 'denis-skripnik', energy: 1000, memo: 'viz://vm/library/chapter2', txIndex: 1 },
      { initiator: 'award-only', receiver: 'denis-skripnik', energy: 1000, memo: 'viz://vm/library/chapter2', txIndex: 2 },
      { initiator: 'mismatched-tx', receiver: 'denis-skripnik', energy: 1000, memo: 'viz://vm/library/chapter2', txIndex: 5 },
      { initiator: 'wrong-memo', receiver: 'denis-skripnik', energy: 1000, memo: 'viz://vm/library/other', txIndex: 6 }
    ]
  });

  assert.strictEqual(engine.hasLibraryAccess('alice', 'chapter2'), true, 'atomic paid unlock should open chapter two');
  assert.strictEqual(engine.getState().libraryAccess.alice.chapter2, 500, 'duplicate replay should stay idempotent and retain the proof block');
  ['award-only', 'action-only', 'mismatched-tx', 'wrong-memo', 'bob'].forEach(function(account) {
    assert.strictEqual(engine.hasLibraryAccess(account, 'chapter2'), false, account + ' must not unlock access');
  });
});


test('magical library chapter two renders fifteen paid secret maps behind one confirmed award', function () {
  assert.ok(/help_magic_library_chapter_two_title:\s*'Магическая Библиотека глава вторая\. Тайные Карты Мира'/.test(ruJs), 'RU title should preserve Denis wording');
  assert.ok(/HELP_SECRET_LIBRARY_MAPS\s*=\s*\[/.test(helpJs), 'Help should define a separate chapter-two map set');
  assert.ok(/function _renderSecretLibrary/.test(helpJs), 'Help should render chapter two separately');
  assert.ok(/StateEngine\.hasLibraryAccess\(user, 'chapter2'\)/.test(helpJs), 'locked UI should use replayed StateEngine entitlement');
  ...
  assert.ok(helpJs.indexOf('HistorySource.findAccountAction') < helpJs.indexOf('VizBroadcast.libraryUnlockAction'), 'archived entitlement preflight must happen before broadcast');
  assert.ok(/verifyLibraryUnlockProof/.test(helpJs), 'archive event must be checked against its full award-plus-custom block');
  assert.ok(/help_secret_library_history_check_failed/.test(helpJs + ruJs + enJs), 'history verification failure should fail closed with explicit copy');
  assert.ok(/VizBroadcast\.libraryUnlockAction\(\s*VizMagicConfig\.LIBRARY\.CHAPTER_TWO_COST/.test(helpJs), 'unlock must use one atomic award plus VM action transaction');
  assert.ok(/function libraryUnlockAction\(energy, callback\)/.test(broadcastJs) && /operations:\s*\[[\s\S]*\['award'[\s\S]*\['custom'/.test(broadcastJs), 'broadcast helper should sign award and VM proof as one transaction');
  assert.ok(/t:\s*cfg\.ACTION_TYPES\.LIBRARY_UNLOCK/.test(broadcastJs) && /b:\s*previous/.test(broadcastJs), 'library unlock proof should join the backward-linked VM action chain');
  assert.ok(/if \(err \|\| !response\)[\s\S]{0,180}protocol_history_unavailable/.test(broadcastJs), 'library unlock must fail closed instead of severing VM history when the previous-link lookup fails');
  assert.ok(/broadcast\.js\?v=20260822h/.test(indexHtml) && /block-processor\.js\?v=20260822i/.test(indexHtml) && /vm-protocol\.js\?v=20260822k/.test(indexHtml), 'atomic broadcast, transaction-index replay and account-safe history traversal should be cache-busted');
  assert.ok(/StateEngine\.processLibraryUnlockResult/.test(helpJs) && /StateEngine\.saveCheckpoint/.test(helpJs), 'successful atomic transaction should update and persist live entitlement');
  assert.ok(/help-secret-library-confirm/.test(helpJs) && /help-secret-library-cancel/.test(helpJs), 'payment should require an explicit confirmation modal');
  assert.ok(!/secret.*minLevel|HELP_SECRET_LIBRARY_MAPS[\s\S]{0,500}minLevel/i.test(helpJs), 'chapter two should not impose level ordering yet');
  assert.strictEqual(fs.readdirSync(path.join(root, 'app/assets/library-maps-chapter2')).filter(name => /^secret-map-\d{2}\.jpg$/.test(name)).length, 15, 'chapter two should contain exactly 15 JPEG maps');
  assert.ok(/assets\/library-maps-chapter2\/secret-map-' \+ entry\.id \+ '\.jpg/.test(helpJs), 'secret-map modal should load chapter-two assets');
});


test('magical library links stay on one line without changing map images', function () {
  assert.ok(/main.css\?v=20260822f/.test(indexHtml), 'CSS must be cache-busted for Magical Library link sizing');
...
test('verified world maps and font-independent SVG icons remain intact', function () {
  ...
  assert.ok(/MAP_ASSET_VERSION = '20260826u'/.test(mapJs), 'travel maps should remain on the verified portal artwork');
  assert.ok(/help\.js\?v=20260822o/.test(indexHtml), 'Help bundle should be cache-busted for text-only library mode');
  assert.ok(/viz-magic-v168/.test(swJs), 'service worker cache should advance for text-only library mode');
  assert.ok(/main\.css\?v=20260822f/.test(indexHtml), 'SVG icon styles should stay cache-busted');
});
...
test('Magical Library uses the original board-game artwork with lore and Close', function () {
  ...
  assert.ok(/help\.js\?v=20260822o/.test(indexHtml), 'restored-art Help bundle should be cache-busted');
  assert.ok(/viz-magic-v168/.test(swJs), 'service worker cache should advance for restored board-game artwork');
});
