...
test('large stale checkpoint catch-up uses archive events instead of replaying empty blocks', function () {
  ...
  assert.ok(/js\/ui\/app\.js\?v=20260823a/.test(indexHtml), 'main app controller must be cache-busted when catch-up code changes');
});
...
test('magical weather is labelled and affects hunts', function () {
  ...
  assert.ok(/i18n\/ru.js\?v=20260823a/.test(indexHtml), 'Russian weather copy must be cache-busted');
  assert.ok(/i18n\/en.js\?v=20260823a/.test(indexHtml), 'English weather copy must be cache-busted');
  assert.ok(/home.js\?v=20260823a/.test(indexHtml), 'home forecast layout must be cache-busted');
  assert.ok(/js\/ui\/screens\/quests.js\?v=20260823a/.test(indexHtml), 'quest-limit UX must be cache-busted');
  assert.ok(/nav.js\?v=20260823a/.test(indexHtml), 'bottom tray nav must be cache-busted');
  assert.ok(/leaderboard.js\?v=20260823a/.test(indexHtml), 'leaderboard icon motion must be cache-busted');
  assert.ok(/world-events.js\?v=20260823a/.test(indexHtml), 'world events news and festival copy must be cache-busted');
  assert.ok(/main.css\?v=20260823a/.test(indexHtml), 'forecast grid CSS must be cache-busted');
...
test('music volume, narrator speech, and PWA icons are durable', function () {
  ...
  assert.ok(/manifest\.json\?v=20260823a/.test(indexHtml), 'manifest should be cache-busted for updated icon');
  assert.ok(/favicon\.ico\?v=20260823a/.test(indexHtml), 'favicon should be explicit for browser shortcut fallback');
  assert.ok(/viz-magic-v133-192\.png\?v=20260823a/.test(indexHtml), 'launcher icon link should be cache-busted');
...
test('temple tab uses balanced on-chain offerings without direct pay-to-win stats', function () {
  ...
  assert.ok(/screen-temple/.test(indexHtml) && /temple\.js\?v=20260823a/.test(indexHtml), 'temple screen should be loaded and cache-busted');
...
test('PWA icon and HP heart use expressive color accents', function () {
  assert.ok(/viz-magic-v133-192\.png\?v=20260823a/.test(indexHtml), 'PWA icon link should be cache-busted after expressive icon update');
...
test('character screen uses current home-scale vitals and growth explainers', function () {
  assert.ok(/character.js\?v=20260823a/.test(indexHtml), 'character screen should be cache-busted');
...
test('hunt screen exposes explicit camp rest promised by Help', function () {
  assert.ok(/hunt.js\?v=20260823a/.test(indexHtml), 'hunt screen should be cache-busted');
  assert.ok(/broadcast.js\?v=20260731c/.test(indexHtml), 'broadcast helper should be cache-busted for restAction');
  assert.ok(/state-engine.js\?v=20260823a/.test(indexHtml), 'state-engine should be cache-busted for processRestResult');
...
test('narrator voice preferences support gender and timbre', function () {
  ...
  assert.ok(/settings.js\?v=20260823a/.test(indexHtml), 'settings should be cache-busted');
...
test('home action tiles reflect Denis priority order', function () {
  assert.ok(/home.js\?v=20260823a/.test(indexHtml), 'home screen should be cache-busted for action order');
...
test('character vital explainers are placed immediately after their bars', function () {
  assert.ok(/character.js\?v=20260823a/.test(indexHtml), 'character screen should be cache-busted for vital layout');
...
test('minor rift banner explains itself and is actionable', function () {
  assert.ok(/home.js\?v=20260823a/.test(indexHtml), 'home screen should be cache-busted for rift explanation');
...
test('temple offering gives immediate heard-prayer feedback', function () {
  assert.ok(/temple\.js\?v=20260823a/.test(indexHtml), 'temple screen should be cache-busted for offering feedback');
...
test('hunt rest uses home-scale HP values', function () {
  assert.ok(/hunt.js\?v=20260823a/.test(indexHtml), 'hunt screen should be cache-busted for HP display scale');
...
test('marketplace groups identical sellable items and supports quantity listing', function () {
  assert.ok(/marketplace.js\?v=20260823a/.test(indexHtml), 'marketplace screen should be cache-busted');
...
test('hunt and arena icons are distinct', function () {
  assert.ok(/nav.js\?v=20260823a/.test(indexHtml), 'nav should be cache-busted for hunt icon');
...
test('hunt headings and help use updated thematic icons', function () {
  assert.ok(/hunt.js\?v=20260823a/.test(indexHtml), 'hunt screen should be cache-busted for heading icons');
  ...
  assert.ok(/help.js\?v=20260823a/.test(indexHtml), 'help screen should be cache-busted for hunt icon');
...
test('marketplace sell items have semantic item icons', function () {
  assert.ok(/marketplace.js\?v=20260823a/.test(indexHtml), 'marketplace screen should be cache-busted for sell icons');
...
test('world boss UI can enter active window from schedule even without spawn checkpoint', function () {
  assert.ok(/world-boss.js\?v=20260823a/.test(indexHtml), 'world boss screen should be cache-busted for active-window fallback');
  ...
  assert.ok(/state-engine.js\?v=20260823a/.test(indexHtml), 'state engine should be cache-busted for boss attack spawn-block parity');
...
test('developers screen offers optional non-advantage award', function () {
  ...
  assert.ok(/developers.js\?v=20260823a/.test(indexHtml), 'developers screen should be loaded and cache-busted');
  ...
  assert.ok(/app.js\?v=20260823a/.test(indexHtml), 'app controller should be cache-busted for developers route');
...
test('v79 PWA icon removes the yellow outline around the plus', function () {
  ...
  assert.ok(/viz-magic-v133-192\.png\?v=20260823a/.test(indexHtml), 'index should point at the v79 launcher icon');
...
test('Denis feedback item and motion icons are semantic', function () {
  assert.ok(/viz-magic-v133-192\.png\?v=20260823a/.test(indexHtml), 'launcher icon should use the v79 centered launcher icon');
...
test('chronicle draft survives rerenders while feed loads', function () {
  assert.ok(/chronicle.js\?v=20260823a/.test(indexHtml), 'chronicle screen should be cache-busted for draft preservation');
...
test('v82 Denis feedback polish is explicit and cache-busted', function () {
  ...
  assert.ok(/home\.js\?v=20260823a/.test(indexHtml), 'Home should be cache-busted for v82');
  assert.ok(/world-events\.js\?v=20260823a/.test(indexHtml), 'world events should be cache-busted for v82');
  assert.ok(/hunt\.js\?v=20260823a/.test(indexHtml), 'Hunt should be cache-busted for Armageddon lock feedback');
  assert.ok(/settings\.js\?v=20260823a/.test(indexHtml), 'Settings should be cache-busted for sound icons');
...
test('magical guide replaces extra magical pages tab without shuffling practical help', function () {
  ...
  assert.ok(/help\.js\?v=20260823a/.test(indexHtml), 'Help should be cache-busted for guide redesign');
  assert.ok(/main\.css\?v=20260823a/.test(indexHtml), 'main CSS should be cache-busted for guide redesign');
  assert.ok(/viz-magic-v137/.test(swJsV83), 'service worker should use the current v91 cache');
...
test('Denis v91 polish batch keeps quests fair and icons lively', () => {
  ...
  assert.ok(/main\.css\?v=20260823a/.test(indexHtml), 'main CSS should be cache-busted for v91 polish');
  assert.ok(/world-events\.js\?v=20260823a/.test(indexHtml), 'world-events should be cache-busted for v91 polish');
  assert.ok(/quest-system\.js\?v=20260823a/.test(indexHtml) && /quests\.js\?v=20260823a/.test(indexHtml), 'quest engine and UI should be cache-busted');
  assert.ok(/inventory\.js\?v=20260823a/.test(indexHtml) && /marketplace\.js\?v=20260823a/.test(indexHtml), 'item icon screens should be cache-busted');
  assert.ok(/guild\.js\?v=20260823a/.test(indexHtml) && /settings\.js\?v=20260823a/.test(indexHtml), 'guild/settings screens should be cache-busted');
  assert.ok(/viz-magic-v137/.test(swJs), 'service worker should use v91 cache');
});
...
test('profile avatars from VIZ json_metadata are bounded and optional', function () {
  ...
  assert.ok(/daily-leaderboard\.js\?v=20260823a/.test(indexHtml), 'daily leaderboard should be cache-busted');
  assert.ok(/leaderboard\.js\?v=20260823a/.test(indexHtml), 'leaderboard UI should be cache-busted');
  assert.ok(/arena\.js\?v=20260823a/.test(indexHtml), 'arena UI should be cache-busted');
  assert.ok(/duel\.js\?v=20260726b/.test(indexHtml), 'duel UI should be cache-busted');
  assert.ok(/world-boss\.js\?v=20260823a/.test(indexHtml), 'world boss UI should be cache-busted');
  assert.ok(/character\.js\?v=20260823a/.test(indexHtml), 'character UI should be cache-busted');
  assert.ok(/main\.css\?v=20260823a/.test(indexHtml), 'avatar CSS should be cache-busted');
  assert.ok(/viz-magic-v137/.test(swJs), 'service worker should use v92 cache');
});
...
test('safe avatar upload UI re-encodes before JSON_METADATA writes', function () {
  ...
  assert.ok(/settings\.js\?v=20260823a/.test(indexHtml), 'settings screen should be cache-busted for avatar upload');
  assert.ok(/js\/i18n\/ru\.js\?v=20260823a/.test(indexHtml) && /js\/i18n\/en\.js\?v=20260823a/.test(indexHtml), 'i18n should be cache-busted for avatar strings');
});
...
test('character profile refreshes avatar from VIZ JSON_METADATA on render', function () {
  ...
  assert.ok(/character\.js\?v=20260823a/.test(indexHtmlAvatarProfile), 'character screen should be cache-busted for live avatar refresh');
  assert.ok(/js\/i18n\/ru\.js\?v=20260823a/.test(indexHtmlAvatarProfile) && /js\/i18n\/en\.js\?v=20260823a/.test(indexHtmlAvatarProfile), 'i18n should be cache-busted for updated avatar copy');
});
...
test('Denis v97 world, inventory, nav, arena and guide polish is explicit', function () {
  ...
  assert.ok(/world-events\.js\?v=20260823a/.test(indexV97) && /home\.js\?v=20260823a/.test(indexV97) && /main\.css\?v=20260823a/.test(indexV97), 'v97 files should be cache-busted');
});
...
test('public landing does not block startup behind chain sync', function () {
  ...
  assert.ok(/js\/ui\/app\.js\?v=20260823a/.test(indexV98), 'app controller should be cache-busted for startup fix');
});
...
test('saved sessions render Home before account and chain sync finish', function () {
  ...
  assert.ok(/js\/ui\/app\.js\?v=20260823a/.test(indexV99), 'app controller should be cache-busted for saved-session launch fix');
});
...
test('living pages use an existing daily page helper', function () {
  ...
  assert.ok(/world-events\.js\?v=20260823a/.test(indexV99), 'world-events should be cache-busted for living-page crash fix');
});
...
test('background sync never shows the sync chip over an active game screen', function () {
  ...
  assert.ok(/js\/ui\/app\.js\?v=20260823a/.test(indexV100), 'app controller should be cache-busted for non-blocking sync status');
});
...
test('startup routes from saved session before network and IndexedDB', function () {
  ...
  assert.ok(/js\/ui\/app\.js\?v=20260823a/.test(indexV101), 'app controller should be cache-busted for immediate saved-session routing');
});
...
test('home startup does not fetch blockchain account before VIZ transport is ready', function () {
  ...
  assert.ok(/home\.js\?v=20260823a/.test(indexV101), 'Home screen should be cache-busted for safe startup mana refresh');
});
...
test('bottom navigation keeps icons above single-line labels and cache-busts changed assets', function () {
  ...
  assert.ok(/main\.css\?v=20260823a/.test(indexHtml), 'main.css cache bust missing');
  assert.ok(/hunt\.js\?v=20260823a/.test(indexHtml), 'hunt cache bust missing');
  assert.ok(/inventory\.js\?v=20260823a/.test(indexHtml), 'inventory cache bust missing');
  assert.ok(/marketplace\.js\?v=20260823a/.test(indexHtml), 'marketplace cache bust missing');
  assert.ok(/nav\.js\?v=20260823a/.test(indexHtml), 'nav cache bust missing');
  assert.ok(/viz-magic-v137/.test(read('app/sw.js')), 'service worker cache should be v103');
});
...
test('v103 weave surge, default avatars, and guide copy polish are explicit', function () {
  ...
  assert.ok(/main\.css\?v=20260823a/.test(indexHtml), 'v103 CSS should be cache-busted');
  assert.ok(/home\.js\?v=20260823a/.test(indexHtml) && /character\.js\?v=20260823a/.test(indexHtml), 'v103 Home and Character should be cache-busted');
  assert.ok(/settings\.js\?v=20260823a/.test(indexHtml) && /help\.js\?v=20260823a/.test(indexHtml), 'v103 Settings and Help should be cache-busted');
  assert.ok(/viz-magic-v137/.test(swJs), 'service worker should use v103 cache');
});
...
test('quest abandon charges only unstarted quests and warns before forfeit', function () {
  ...
  assert.ok(/quest-system\.js\?v=20260823a/.test(indexPenalty), 'quest system cache bust missing for abandon penalty');
  assert.ok(/state-engine\.js\?v=20260823a/.test(indexPenalty), 'state engine cache bust missing for abandon penalty');
  assert.ok(/quests\.js\?v=20260823a/.test(indexPenalty), 'quests screen cache bust missing for abandon penalty');
  assert.ok(/js\/i18n\/ru\.js\?v=20260823a/.test(indexPenalty) && /js\/i18n\/en\.js\?v=20260823a/.test(indexPenalty), 'i18n cache bust missing for abandon penalty');
  assert.ok(/viz-magic-v137/.test(swPenalty), 'service worker should use v104 cache');
});
...
test('v110 player feedback keeps requested icons, copy, vital explainers, and event text placement explicit', function () {
  ...
  ['main.css','ru.js','en.js','world-events.js','progress-bar.js','nav.js','home.js','character.js','hunt.js','inventory.js','arena.js','settings.js','leaderboard.js'].forEach(function(asset) {
    assert.ok(new RegExp(asset.replace('.', '\\.') + '\\?v=20260823a').test(index), asset + ' should be cache-busted for v110');
  });
  assert.ok(/viz-magic-v137/.test(sw), 'service worker should use v110 cache');
});
...
test('v109 player polish batch removes stale text, fixes motion/copy/icons, and fills hunt tier gap', function () {
  ...
  ['main.css','ru.js','en.js','world-events.js','creatures.js','character.js','hunt.js','inventory.js','arena.js','guild.js','marketplace.js','leaderboard.js','settings.js'].forEach(function(asset) {
    assert.ok(new RegExp(asset.replace('.', '\\.') + '\\?v=20260823a').test(indexV109), asset + ' should be cache-busted for v109');
  });
  assert.ok(/viz-magic-v137/.test(swV109), 'service worker should use v109 cache');
});
...
test('v108 inventory, bottom nav, hunt danger marker, and arena motion polish are explicit', function () {
  ...
  assert.ok(/main\.css\?v=20260823a/.test(indexV108), 'main CSS cache bust missing for v108');
  assert.ok(/inventory\.js\?v=20260823a/.test(indexV108), 'inventory cache bust missing for v108');
  assert.ok(/hunt\.js\?v=20260823a/.test(indexV108), 'hunt cache bust missing for v108');
  assert.ok(/arena\.js\?v=20260823a/.test(indexV108), 'arena cache bust missing for v108');
  assert.ok(/nav\.js\?v=20260823a/.test(indexV108), 'nav cache bust missing for v108');
  assert.ok(/marketplace\.js\?v=20260823a/.test(indexV108), 'marketplace cache bust missing for v108 icon parity');
  assert.ok(/js\/i18n\/ru\.js\?v=20260823a/.test(indexV108) && /js\/i18n\/en\.js\?v=20260823a/.test(indexV108), 'i18n cache bust missing for v108 item labels');
  assert.ok(/viz-magic-v137/.test(swV108), 'service worker should use v108 cache');
});
...
test('v107 hunt combat uses spell mana cost, not full account energy', function () {
  ...
  assert.ok(/combat\.js\?v=20260823a/.test(indexV107), 'combat cache bust missing for v118 hunt mercy fix');
  assert.ok(/state-engine\.js\?v=20260823a/.test(indexV107), 'state engine cache bust missing for v107 hunt energy fix');
  assert.ok(/hunt\.js\?v=20260823a/.test(indexV107), 'hunt screen cache bust missing for v107 hunt energy fix');
  assert.ok(/viz-magic-v137/.test(swV107), 'service worker should use v107 cache');
});
...
test('v106 hunt danger copy is truthful and Weave title is single-line', function () {
  ...
  assert.ok(/main\.css\?v=20260823a/.test(indexV106), 'v106 CSS cache bust missing');
  assert.ok(/creatures\.js\?v=20260823a/.test(indexV106), 'v106 creatures cache bust missing');
  assert.ok(/hunt\.js\?v=20260823a/.test(indexV106), 'v106 hunt cache bust missing');
  assert.ok(/viz-magic-v137/.test(swV106), 'service worker should use v106 cache');
});
...
test('v105 text quality, season colors, avatar title, and spell modal polish are explicit', function () {
  ...
  assert.ok(/main\.css\?v=20260823a/.test(indexV105), 'main CSS cache bust missing for v105');
  assert.ok(/home\.js\?v=20260823a/.test(indexV105) && /character\.js\?v=20260823a/.test(indexV105), 'Home/Character cache bust missing for v105');
  assert.ok(/settings\.js\?v=20260823a/.test(indexV105) && /leaderboard\.js\?v=20260823a/.test(indexV105), 'Settings/Leaderboard cache bust missing for v105');
  assert.ok(/world-events\.js\?v=20260823a/.test(indexV105), 'world events cache bust missing for v105');
  assert.ok(/js\/i18n\/ru\.js\?v=20260823a/.test(indexV105) && /js\/i18n\/en\.js\?v=20260823a/.test(indexV105), 'i18n cache bust missing for v105');
  assert.ok(/viz-magic-v137/.test(swV105), 'service worker should use v105 cache');
});
...
test('v112 player feedback fixes event surface weather inventory and ranking polish', function () {
  ...
  ['main.css','ru.js','en.js','world-events.js','progress-bar.js','nav.js','home.js','character.js','hunt.js','inventory.js','arena.js','settings.js','leaderboard.js','marketplace.js','guild.js','creatures.js'].forEach(function(asset) {
    assert.ok(new RegExp(asset.replace('.', '\\.') + '\\?v=20260823a').test(index), asset + ' should be cache-busted for v112');
  });
  assert.ok(/viz-magic-v137/.test(sw), 'service worker should use v112 cache');
});
...
test('v114 loading state does not invent an embercaster level-one character', function () {
  ...
  assert.ok(/home.js\?v=20260823a/.test(indexHtml) && /character.js\?v=20260823a/.test(indexHtml), 'loading fallback fix should be cache-busted');
  assert.ok(/viz-magic-v137/.test(swJs), 'service worker should publish v114');
});


test('v115 service worker forces stale PWA windows onto the fresh cache-busted app shell', function () {
  assert.ok(/app.js\?v=20260823a/.test(indexHtml), 'main app controller should be cache-busted with UI fixes');
  ...
  assert.ok(/viz-magic-v137/.test(swJs), 'service worker should publish v115');
});
...
test('v116 character tile does not use ember fallback while the real character is loading', function () {
  ...
  assert.ok(/home.js\?v=20260823a/.test(indexHtml), 'character tile loading fix should be cache-busted');
});
