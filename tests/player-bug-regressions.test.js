...
test('large stale checkpoint catch-up uses archive events instead of replaying empty blocks', function () {
  ...
  assert.ok(/js\/ui\/app\.js\?v=20260826u/.test(indexHtml), 'main app controller must be cache-busted when catch-up code changes');
});
...
test('magical weather is labelled and affects hunts', function () {
  ...
  assert.ok(/main.css\?v=20260826u/.test(indexHtml), 'forecast grid CSS must be cache-busted');
...
test('hunt screen exposes explicit camp rest promised by Help', function () {
  ...
  assert.ok(/state-engine.js\?v=20260826u/.test(indexHtml), 'state-engine should be cache-busted for processRestResult');
...
test('hunt headings and help use updated thematic icons', function () {
  ...
  assert.ok(/help.js\?v=20260826w/.test(indexHtml), 'help screen should be cache-busted for hunt icon');
...
test('world boss UI can enter active window from schedule even without spawn checkpoint', function () {
  ...
  assert.ok(/state-engine.js\?v=20260826u/.test(indexHtml), 'state engine should be cache-busted for boss attack spawn-block parity');
...
test('developers screen offers optional non-advantage award', function () {
  ...
  assert.ok(/app.js\?v=20260826u/.test(indexHtml), 'app controller should be cache-busted for developers route');
...
test('magical library preserves legacy board-game maps in the living guide pages', function () {
  ...
  assert.ok(/assets\/library-maps-v2\/map-' \+ entry\.id \+ '\.jpg\?v=' \+ HELP_LIBRARY_ASSET_VERSION/.test(helpJs), 'library modal should load clean maps from the versioned folder');
  ...
  assert.ok(/help\.js\?v=20260826w/.test(index), 'help screen should be cache-busted for the Magical Library');
  assert.ok(/main\.css\?v=20260826u/.test(index), 'CSS should be cache-busted for the Magical Library');
  ...
  assert.ok(/viz-magic-v161/.test(sw), 'service worker should publish the Magical Library cache bump');
  ids.forEach(function(id) {
    assert.ok(fs.existsSync(path.join(root, 'app/assets/library-maps-v2/map-' + id + '.jpg')), 'legacy library map should exist for ' + id);
  });
});
...
test('magical guide replaces extra magical pages tab without shuffling practical help', function () {
  ...
  assert.ok(/help\.js\?v=20260826w/.test(indexHtml), 'Help should be cache-busted for guide redesign');
  assert.ok(/main\.css\?v=20260826u/.test(indexHtml), 'main CSS should be cache-busted for guide redesign');
  assert.ok(/viz-magic-v161/.test(swJsV83), 'service worker should use the current v91 cache');
...
test('Denis v91 polish batch keeps quests fair and icons lively', () => {
  ...
  assert.ok(/main\.css\?v=20260826u/.test(indexHtml), 'main CSS should be cache-busted for v91 polish');
  ...
  assert.ok(/viz-magic-v161/.test(swJs), 'service worker should use v91 cache');
});
...
test('profile avatars from VIZ json_metadata are bounded and optional', function () {
  ...
  assert.ok(/main\.css\?v=20260826u/.test(indexHtml), 'avatar CSS should be cache-busted');
  assert.ok(/viz-magic-v161/.test(swJs), 'service worker should use v92 cache');
});
...
test('Denis v97 world, inventory, nav, arena and guide polish is explicit', function () {
  ...
  assert.ok(/world-events\.js\?v=20260826k/.test(indexV97) && /home\.js\?v=20260826u/.test(indexV97) && /main\.css\?v=20260826u/.test(indexV97), 'v97 files should be cache-busted');
});
...
test('public landing does not block startup behind chain sync', function () {
  ...
  assert.ok(/js\/ui\/app\.js\?v=20260826u/.test(indexV98), 'app controller should be cache-busted for startup fix');
});
...
test('saved sessions render Home before account and chain sync finish', function () {
  ...
  assert.ok(/js\/ui\/app\.js\?v=20260826u/.test(indexV99), 'app controller should be cache-busted for saved-session launch fix');
});
...
test('background sync never shows the sync chip over an active game screen', function () {
  ...
  assert.ok(/js\/ui\/app\.js\?v=20260826u/.test(indexV100), 'app controller should be cache-busted for non-blocking sync status');
});
...
test('startup routes from saved session before network and IndexedDB', function () {
  ...
  assert.ok(/js\/ui\/app\.js\?v=20260826u/.test(indexV101), 'app controller should be cache-busted for immediate saved-session routing');
});
...
test('bottom navigation keeps icons above single-line labels and cache-busts changed assets', function () {
  ...
  assert.ok(/main\.css\?v=20260826u/.test(indexHtml), 'main.css cache bust missing');
  ...
  assert.ok(/viz-magic-v161/.test(read('app/sw.js')), 'service worker cache should be v103');
});
...
test('v103 weave surge, default avatars, and guide copy polish are explicit', function () {
  ...
  assert.ok(/main\.css\?v=20260826u/.test(indexHtml), 'v103 CSS should be cache-busted');
  ...
  assert.ok(/settings\.js\?v=20260826k/.test(indexHtml) && /help\.js\?v=20260826w/.test(indexHtml), 'v103 Settings and Help should be cache-busted');
  assert.ok(/viz-magic-v161/.test(swJs), 'service worker should use v103 cache');
});
...
test('quest abandon charges only unstarted quests and warns before forfeit', function () {
  ...
  assert.ok(/state-engine\.js\?v=20260826u/.test(indexPenalty), 'state engine cache bust missing for abandon penalty');
  ...
  assert.ok(/viz-magic-v161/.test(swPenalty), 'service worker should use v104 cache');
});
...
test('v110 player feedback keeps requested icons, copy, vital explainers, and event text placement explicit', function () {
  ...
  ['main.css','ru.js','en.js','world-events.js','progress-bar.js','nav.js','home.js','character.js','hunt.js','inventory.js','arena.js','settings.js','leaderboard.js'].forEach(function(asset) {
    var version = asset === 'main.css' ? '20260826u' : ((asset === 'home.js' || asset === 'character.js' || asset === 'nav.js' || asset === 'hunt.js') ? '20260826u' : ((asset === 'ru.js' || asset === 'en.js') ? '20260826q' : '20260826k'));
    assert.ok(new RegExp(asset.replace('.', '\\.') + '\\?v=' + version).test(index), asset + ' should be cache-busted for v110');
  });
  assert.ok(/viz-magic-v161/.test(sw), 'service worker should use v110 cache');
});
...
test('v109 player polish batch removes stale text, fixes motion/copy/icons, and fills hunt tier gap', function () {
  ...
  assert.ok(/viz-magic-v161/.test(swV109), 'service worker should use v109 cache');
});
...
test('v108 inventory, bottom nav, hunt danger marker, and arena motion polish are explicit', function () {
  ...
  assert.ok(/main\.css\?v=20260826u/.test(indexV108), 'main CSS cache bust missing for v108');
  ...
  assert.ok(/viz-magic-v161/.test(swV108), 'service worker should use v108 cache');
});
...
test('v107 hunt combat uses spell mana cost, not full account energy', function () {
  ...
  assert.ok(/state-engine\.js\?v=20260826u/.test(indexV107), 'state engine cache bust missing for v107 hunt energy fix');
  assert.ok(/hunt\.js\?v=20260826u/.test(indexV107), 'hunt screen cache bust missing for v107 hunt energy fix');
  assert.ok(/viz-magic-v161/.test(swV107), 'service worker should use v107 cache');
});
...
test('v106 hunt danger copy is truthful and Weave title is single-line', function () {
  ...
  assert.ok(/main\.css\?v=20260826u/.test(indexV106), 'v106 CSS cache bust missing');
  ...
  assert.ok(/viz-magic-v161/.test(swV106), 'service worker should use v106 cache');
});
...
test('v105 text quality, season colors, avatar title, and spell modal polish are explicit', function () {
  ...
  assert.ok(/main\.css\?v=20260826u/.test(indexV105), 'main CSS cache bust missing for v105');
  ...
  assert.ok(/viz-magic-v161/.test(swV105), 'service worker should use v105 cache');
});
...
test('v112 player feedback fixes event surface weather inventory and ranking polish', function () {
  ...
  assert.ok(/viz-magic-v161/.test(sw), 'service worker should use v112 cache');
});
...
test('v114 loading state does not invent an embercaster level-one character', function () {
  ...
  assert.ok(/viz-magic-v161/.test(swJs), 'service worker should publish v114');
});


test('v115 service worker forces stale PWA windows onto the fresh cache-busted app shell', function () {
  assert.ok(/app.js\?v=20260826u/.test(indexHtml), 'main app controller should be cache-busted with UI fixes');
  ...
  assert.ok(/viz-magic-v161/.test(swJs), 'service worker should publish v115');
});
...
test('production PWA install is available before login and landing icons do not depend on emoji fonts', function () {
  ...
  assert.ok(/viz-magic-v161/.test(swJs), 'service-worker cache should advance for fresh runtime assets');
});
...
test('installed app cold-start has visible fallback and service-worker network timeouts', function () {
  ...
  assert.ok(/viz-magic-v161/.test(swJs), 'service worker cache should advance for cold-start black-screen fix');
...
test('critical desktop icons avoid emoji glyph squares on Home and bottom nav', function () {
  ...
  assert.ok(/main\.css\?v=20260826u/.test(indexHtml), 'CSS must be cache-busted for icon replacement');
...
test('magical library links stay on one line without changing map images', function () {
  assert.ok(/main.css\?v=20260826u/.test(indexHtml), 'CSS must be cache-busted for Magical Library link sizing');
...
test('portal-free library maps and font-independent SVG icons replace failed retouch', function () {
  const mainCss = read('app/css/main.css');
  const mapJs = read('app/js/ui/screens/map.js');
  const helpJs = read('app/js/ui/screens/help.js');
  ...
  assert.ok(/HELP_LIBRARY_ASSET_VERSION = '20260817b'/.test(helpJs), 'library maps should have a fresh cache identity');
  assert.ok(helpJs.includes("assets/library-maps-v2/map-"), 'library maps should use a new physical asset path');
  assert.ok(swJs.includes("url.pathname.indexOf('/assets/library-maps-v2/') === 0") && swJs.includes("url.pathname.indexOf('/assets/library-maps/') === 0"), 'service worker should refresh both versioned and legacy library map URLs');
  assert.ok(swJs.includes("isMapImage || isLibraryMapImage"), 'library maps should use the map network-first branch');
  assert.ok(/_fetchWithTimeout\(event\.request, RUNTIME_TIMEOUT_MS, \{ cache: 'reload' \}\)/.test(swJs), 'map images should bypass stale HTTP cache');
  assert.ok(/help\.js\?v=20260826w/.test(indexHtml), 'Help bundle should be cache-busted for the new library path');
  assert.ok(/viz-magic-v161/.test(swJs), 'service worker cache should advance after the library cache fix');
  assert.ok(/main\.css\?v=20260826u/.test(indexHtml), 'SVG icon styles should stay cache-busted');
});
