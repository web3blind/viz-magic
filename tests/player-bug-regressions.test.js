...
test('large stale checkpoint catch-up uses archive events instead of replaying empty blocks', function () {
  ...
  assert.ok(/js\/ui\/app\.js\?v=20260720b/.test(indexHtml), 'main app controller must be cache-busted when catch-up code changes');
});
...
test('service worker updates quickly and keeps navigations network-first', function () {
  ...
  assert.ok(/viz-magic-v77/.test(swJs), 'service worker cache version should be bumped');
...
test('mobile entry helpers cover keyboard paste, home-screen shortcut, nav parity, non-intrusive toasts, and chronicle post dedupe', function () {
  ...
  assert.ok(/viz-magic-v77/.test(read('app/sw.js')), 'service worker cache should be bumped for UI changes');
});


test('home dashboard puts mana first and uses a story-readable HP scale', function () {
  assert.ok(/HOME_HP_DISPLAY_FACTOR = 10/.test(homeJs), 'home HP should use a readable 100 → 1000 display scale');
  ...
  assert.ok(/function _scaleHpForDisplay/.test(homeJs), 'HP display scale should be calculated separately from real combat HP');
  assert.ok(/displayMax:hpDisplay\.max/.test(homeJs), 'HP bar should show the calculated visual HP scale');
...
test('magical weather is labelled and affects hunts', function () {
  ...
  assert.ok(/i18n\/ru.js\?v=20260720b/.test(indexHtml), 'Russian weather copy must be cache-busted');
  assert.ok(/i18n\/en.js\?v=20260720b/.test(indexHtml), 'English weather copy must be cache-busted');
  assert.ok(/home.js\?v=20260720b/.test(indexHtml), 'home forecast layout must be cache-busted');
  assert.ok(/js\/ui\/screens\/quests.js\?v=20260720b/.test(indexHtml), 'quest-limit UX must be cache-busted');
  ...
  assert.ok(/world-events.js\?v=20260718c/.test(indexHtml), 'world events forecast pool must be cache-busted');
  assert.ok(/main.css\?v=20260720b/.test(indexHtml), 'forecast grid CSS must be cache-busted');
...
test('music volume, narrator speech, and PWA icons are durable', function () {
  ...
  assert.ok(/manifest\.json\?v=20260720b/.test(indexHtml), 'manifest should be cache-busted for updated icon');
  assert.ok(/favicon\.ico\?v=20260720b/.test(indexHtml), 'favicon should be explicit for browser shortcut fallback');
  assert.ok(/viz-magic-v77-192\.png\?v=20260720b/.test(indexHtml), 'launcher icon link should be cache-busted');
  assert.ok(/assets\/icons\/viz-magic-v77-512\.png/.test(read('app/sw.js')), 'service worker should cache PWA launcher icons');
  assert.ok(/viz-magic-v77-512\.png/.test(read('app/manifest.json')), 'manifest should reference new icon URLs to bypass OS icon cache');
});
...
test('PWA icon and HP heart use expressive color accents', function () {
  assert.ok(/viz-magic-v77-192\.png\?v=20260720b/.test(indexHtml), 'PWA icon link should be cache-busted after expressive icon update');
  assert.ok(/viz-magic-v77/.test(read('app/manifest.json')), 'manifest start URL should change so launchers can refresh icons');
...
test('character screen uses current home-scale vitals and growth explainers', function () {
  assert.ok(/character.js\?v=20260720b/.test(indexHtml), 'character screen should be cache-busted');
  assert.ok(/CHARACTER_HP_DISPLAY_FACTOR = 10/.test(characterScreenJs), 'character HP should use the same 100 → 1000 display factor as Home');
  ...
  assert.ok(/displayMax:hpDisplay\.max/.test(characterScreenJs), 'character HP display should be separate from real combat HP');
...
test('hunt screen exposes explicit camp rest promised by Help', function () {
  assert.ok(/hunt.js\?v=20260718c/.test(indexHtml), 'hunt screen should be cache-busted');
...
test('home action tiles reflect Denis priority order', function () {
  assert.ok(/home.js\?v=20260720b/.test(indexHtml), 'home screen should be cache-busted for action order');
...
test('character vital explainers are placed immediately after their bars', function () {
  assert.ok(/character.js\?v=20260720b/.test(indexHtml), 'character screen should be cache-busted for vital layout');
...
test('minor rift banner explains itself and is actionable', function () {
  assert.ok(/home.js\?v=20260720b/.test(indexHtml), 'home screen should be cache-busted for rift explanation');
...
test('hunt rest uses home-scale HP values', function () {
  assert.ok(/hunt.js\?v=20260718c/.test(indexHtml), 'hunt screen should be cache-busted for HP display scale');
...
test('marketplace groups identical sellable items and supports quantity listing', function () {
  assert.ok(/marketplace.js\?v=20260719a/.test(indexHtml), 'marketplace screen should be cache-busted');
...
test('hunt headings and help use updated thematic icons', function () {
  assert.ok(/hunt.js\?v=20260718c/.test(indexHtml), 'hunt screen should be cache-busted for heading icons');
...
test('marketplace sell items have semantic item icons', function () {
  assert.ok(/marketplace.js\?v=20260719a/.test(indexHtml), 'marketplace screen should be cache-busted for sell icons');
...
test('developers screen offers optional non-advantage award', function () {
  ...
  assert.ok(/developers.js\?v=20260720b/.test(indexHtml), 'developers screen should be loaded and cache-busted');
  ...
  assert.ok(/app.js\?v=20260720b/.test(indexHtml), 'app controller should be cache-busted for developers route');
...
test('v77 PWA icon keeps previous launcher visual with thinner yellow outline', function () {
  assert.ok(fs.existsSync(path.join(root, 'app/assets/icons/viz-magic-v77-192.png')), 'v77 192px thinner-outline icon should exist');
  assert.ok(fs.existsSync(path.join(root, 'app/assets/icons/viz-magic-v77-512.png')), 'v77 512px thinner-outline icon should exist');
  assert.ok(/viz-magic-v77-192\.png\?v=20260720b/.test(indexHtml), 'index should point at the v77 launcher icon');
  assert.ok(/"start_url":\s*"\/\?pwa=viz-magic-v77"/.test(read('app/manifest.json')), 'manifest start_url should force OS launcher refresh');
  assert.ok(/"id":\s*"https:\/\/vizmagic\.web3blind\.xyz\/\?pwa=viz-magic-v77"/.test(read('app/manifest.json')), 'manifest id should change so Android can refresh launcher identity');
  assert.ok(/APP_SHELL_ASSETS[\s\S]*viz-magic-v77-512\.png/.test(read('app/sw.js')), 'fast install shell should still include current launcher icons');
});

test('Denis feedback item and motion icons are semantic', function () {
  assert.ok(/viz-magic-v77-192\.png\?v=20260720b/.test(indexHtml), 'launcher icon should use the v77 thinner-yellow-outline launcher icon');
  assert.ok(/assets\/icons\/viz-magic-v77-512\.png/.test(read('app/sw.js')), 'service worker should cache v77 icon');
...
test('chronicle draft survives rerenders while feed loads', function () {
  assert.ok(/chronicle.js\?v=20260720b/.test(indexHtml), 'chronicle screen should be cache-busted for draft preservation');
