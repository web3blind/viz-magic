...
test('hunt and map UX fixes prevent known loops and confusing copy', function () {
  ...
  assert.ok(/hunt_choose_spell[\s\S]*hunt_choose_power_continuation/.test(huntJs), 'hunt heading should continue with hunt power on the next line');
  ...
  assert.ok(/hunt_power_cautious_desc/.test(huntJs + ruJs + enJs) && /минимальный расход, риск выше/.test(ruJs), 'per-button hunt descriptions should remain available for non-visible accessible labels');
  assert.ok(!/hunt_min_mana_hint|hunt_choose_power:|hunt_power_hint|hunt-power-desc|<span class="hunt-power-desc"/.test(huntJs + ruJs + enJs + mainCss), 'extra visible hunt-power paragraphs and per-button description lines should stay removed');
  ...
  assert.ok(/css\/accessibility\.css\?v=20260902a/.test(indexHtml), 'accessibility CSS should be cache-busted after removing old spell button focus styles');
...
test('hunt screen exposes explicit camp rest promised by Help', function () {
  assert.ok(/hunt.js\?v=20260826z/.test(indexHtml), 'hunt screen should be cache-busted');
...
test('hunt rest uses home-scale HP values', function () {
  assert.ok(/hunt.js\?v=20260826z/.test(indexHtml), 'hunt screen should be cache-busted for HP display scale');
...
test('hunt headings and help use updated thematic icons', function () {
  assert.ok(/hunt.js\?v=20260826z/.test(indexHtml), 'hunt screen should be cache-busted for heading icons');
...
test('map regions separate safe travel from gated current-region exploration', function () {
  ...
  assert.ok(/main.css\?v=20260824a-20260828j/.test(indexHtml), 'map action styles should be cache-busted');
  ...
  assert.ok(/js\/i18n\/ru.js\?v=20260824f-20260827p-20260828t/.test(indexHtml) && /js\/i18n\/en.js\?v=20260824f-20260827p-20260828s/.test(indexHtml), 'map status translations should be cache-busted');
  assert.ok(/viz-magic-v210/.test(swJs), 'service worker cache should advance for the map action layout');
});
...
test('map travel hint links to a permanent travel and exploration guide page', function () {
  ...
  assert.ok(/data-help-nav="crafting">Мастерской<\/button><br><br>0,10% энергии/.test(ruJs), 'the introductory Workshop button should not be followed by a dot');
  assert.ok(!/data-help-nav="crafting">Мастерской<\/button>\.<br><br>0,10% энергии/.test(ruJs), 'the dot after the Workshop button should be removed');
...
test('magical guide explains versioned XP thresholds without claiming a 100-percent mana level-up', function () {
  ...
  assert.ok(/Текущий прогресс XP показан на страницах <button type="button" class="help-nav-link" data-help-nav="home">Дом<\/button> и <button type="button" class="help-nav-link" data-help-nav="character">Персонаж<\/button>\./.test(ruJs), 'Home and Character should be keyboard-safe navigation links');
...
test('v82 Denis feedback polish is explicit and cache-busted', function () {
  ...
  assert.ok(/hunt\.js\?v=20260826z/.test(indexHtml), 'Hunt should be cache-busted for Armageddon lock feedback');
...
test('magical library restores the original board-game artwork with full lore', function () {
  ...
  assert.ok(/viz-magic-v210/.test(sw), 'service worker should publish the restored library cache bump');
});
...
test('magical guide replaces extra magical pages tab without shuffling practical help', function () {
  ...
  assert.ok(/viz-magic-v210/.test(swJsV83), 'service worker should use the current v91 cache');
...
test('Denis v91 polish batch keeps quests fair and icons lively', () => {
  ...
  assert.ok(/viz-magic-v210/.test(swJs), 'service worker should use v91 cache');
});
...
test('profile avatars from VIZ json_metadata are bounded and optional', function () {
  ...
  assert.ok(/viz-magic-v210/.test(swJs), 'service worker should use v92 cache');
});
...
test('bottom navigation keeps icons above single-line labels and cache-busts changed assets', function () {
  ...
  assert.ok(/hunt\.js\?v=20260826z/.test(indexHtml), 'hunt cache bust missing');
  ...
  assert.ok(/viz-magic-v210/.test(read('app/sw.js')), 'service worker cache should be v103');
});
...
test('v103 weave surge, default avatars, and guide copy polish are explicit', function () {
  ...
  assert.ok(/viz-magic-v210/.test(swJs), 'service worker should use v103 cache');
});
...
test('quest abandon charges only unstarted quests and warns before forfeit', function () {
  ...
  assert.ok(/viz-magic-v210/.test(swPenalty), 'service worker should use v104 cache');
});
...
test('v110 player feedback keeps requested icons, copy, vital explainers, and event text placement explicit', function () {
  ...
  ['main.css','ru.js','en.js','world-events.js','progress-bar.js','nav.js','home.js','character.js','hunt.js','inventory.js','arena.js','settings.js','leaderboard.js'].forEach(function(asset) {
    var version = asset === 'main.css' ? '20260824a' : (asset === 'hunt.js' ? '20260826z' : (asset === 'creatures.js' ? '20260826l' : ((asset === 'home.js' || asset === 'character.js' || asset === 'nav.js') ? '20260826u' : ((asset === 'ru.js' || asset === 'en.js') ? '20260824f' : '20260826k'))));
    assert.ok(new RegExp(asset.replace('.', '\\.') + '\\?v=' + version).test(index), asset + ' should be cache-busted for v110');
  });
  assert.ok(/viz-magic-v210/.test(sw), 'service worker should use v110 cache');
});
...
test('v109 player polish batch removes stale text, fixes motion/copy/icons, and fills hunt tier gap', function () {
  ...
  assert.ok(/viz-magic-v210/.test(swV109), 'service worker should use v109 cache');
});
...
test('v108 inventory, bottom nav, hunt danger marker, and arena motion polish are explicit', function () {
  ...
  assert.ok(/hunt\.js\?v=20260826z/.test(indexV108), 'hunt cache bust missing for v108');
  ...
  assert.ok(/viz-magic-v210/.test(swV108), 'service worker should use v108 cache');
});
...
test('v107 hunt combat uses spell mana cost, not full account energy', function () {
  ...
  assert.ok(/hunt\.js\?v=20260826z/.test(indexV107), 'hunt screen cache bust missing for v107 hunt energy fix');
  assert.ok(/viz-magic-v210/.test(swV107), 'service worker should use v107 cache');
});
...
test('v106 hunt danger copy is truthful and Weave title is single-line', function () {
  ...
  assert.ok(/hunt\.js\?v=20260826z/.test(indexV106), 'v106 hunt cache bust missing');
  assert.ok(/viz-magic-v210/.test(swV106), 'service worker should use v106 cache');
});
...
test('v105 text quality, season colors, avatar title, and spell modal polish are explicit', function () {
  ...
  assert.ok(/viz-magic-v210/.test(swV105), 'service worker should use v105 cache');
});
...
test('v112 player feedback fixes event surface weather inventory and ranking polish', function () {
  ...
  assert.ok(/viz-magic-v210/.test(sw), 'service worker should use v112 cache');
});
...
test('v114 loading state does not invent an embercaster level-one character', function () {
  ...
  assert.ok(/viz-magic-v210/.test(swJs), 'service worker should publish v114');
});
...
test('v115 service worker forces stale PWA windows onto the fresh cache-busted app shell', function () {
  ...
  assert.ok(/viz-magic-v210/.test(swJs), 'service worker should publish v115');
});
...
test('production PWA install is available before login and landing icons do not depend on emoji fonts', function () {
  ...
  assert.ok(/viz-magic-v210/.test(swJs), 'service-worker cache should advance for fresh runtime assets');
});
...
test('installed app cold-start has visible fallback and service-worker network timeouts', function () {
  ...
  assert.ok(/viz-magic-v210/.test(swJs), 'service worker cache should advance for cold-start black-screen fix');
...
test('verified world maps and font-independent SVG icons remain intact', function () {
  ...
  assert.ok(/viz-magic-v210/.test(swJs), 'service worker cache should advance for text-only library mode');
...
test('Magical Library uses the original board-game artwork with lore and Close', function () {
  ...
  assert.ok(/viz-magic-v210/.test(swJs), 'service worker cache should advance for restored board-game artwork');
});
